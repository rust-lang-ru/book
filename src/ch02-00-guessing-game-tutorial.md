...
Теперь давайте соберем программу «Здравствуй мир!» и сразу на этом же шаге запустим её с помощью приказов `cargo run`:
...
Как упоминалось ранее, `read_line` помещает всё, что вводит пользователь, в строку, которую мы ему передаём, но также возвращает значение `Result`. <a data-md-type="raw_html" href="../std/result/enum.Итог.html">`Result`</a><!-- ignore --> — это [*перечисление*]<!-- ignore -->, часто называемое *enum*, то есть вид данных, который может находиться в одном из нескольких возможных состояний. Мы называем каждое такое состояние *исходом*.
...
На данном шаге первая часть игры завершена: мы получаем ввод с клавиатуры и затем выводим его.
...
<!--
The error числа in this output should be that of the code **WITHOUT** the

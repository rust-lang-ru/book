...
В этом случае `len` (длина) будет равна 4, это означает, что вектор, хранит строку "Hola" длиной 4 байта. Каждая из этих букв занимает 1 байт при преобразовании в UTF-8. Но как насчёт следующей строки? (Обратите внимание, что эта строка начинается с заглавной кириллической "З", а не цифры 3.)
...
Таким образом, чтобы предотвратить возврат непредвиденного значения, вызывающего ошибки, которые не могут быть сразу обнаружены, Ржавчина просто не собирает такую рукопись и предотвращает недопонимание на ранних шагах шага разработки.
...
Извлечение кластеров графем из строк, как в случае с языком хинди, является сложным, поэтому эта возможность не предусмотрена встроенной библиотекой. На [crates.io](https://crates.io/)<!-- ignore --> есть доступные библиотеки, которые предоставляют такие возможности.

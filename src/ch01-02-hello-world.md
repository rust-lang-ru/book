...
Тело способа (функции) заключено в `{}`. Ржавчина требует узорчатых скобок вокруг всех тел способов (функций). Принятое внешнее исполнение — поместить открывающую узорчатую скобку на ту же строку, что и объявление способа (функции), добавив между ними один пробел.
...
Тело способа (функции) `main` содержит следующую рукопись:
...
Во-вторых, `println!` вызывается макрос Ржавчины. Если бы вместо него была вызвана способ (функция), она была бы набрана как `println` (без `!`). Более подробно мы обсудим макросы Ржавчина в главе 19. Пока достаточно знать, что использование `!` подразумевает вызов макроса вместо обычного способа (функции), и что макросы не всегда подчиняются тем же правилам как способы (функции).
...
Вы только что запустили впервые созданную программу, поэтому давайте рассмотрим каждый шаг этого шага.

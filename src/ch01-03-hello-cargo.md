...
Почему вам не нужен исполняемый файл? Часто `cargo check` используется намного быстрее, чем `cargo build`, поскольку пропускает шаг создания исполняемого файла. Если вы постоянно проверяете свою работу во время написания рукописи, использование `cargo check` ускорит шаг уведомления вас о том, что ваш ящик всё ещё собирается! Таким образом, многие участники сообщества Ржавчины периодически запускают `cargo check`, когда пишут свои приложения, чтобы убедиться, что она собирается. Затем они запускают `cargo build`, когда готовы использовать исполняемый файл.

...
Первая ошибка находится в нашем выполнении `Drop`. Ранее мы упоминали, что намеревались вызвать `take` для свойства `Option`, чтобы забрать `thread` из шага `worker`. Следующие изменения делают это:
...
Удаление `sender` закрывает поток, что указывает на то, что сообщения больше не будут отправляться. Когда это произойдёт, все вызовы `recv`, выполняемые рабочими шагами в бесконечном круговороте, вернут ошибку. В приложении 20-24 мы меняем круговорот `Worker` для правильного выхода из него в этом случае, это означает, что потоки завершатся, когда использование `drop` `ThreadPool` вызовет для них `join`.
...
Способ `take` определён в сущности `Iterator` и ограничивает повтор самое большее первыми двумя переменными. `ThreadPool` выйдет из области видимости в конце `main` и будет запущен способ `drop`.
...
<!-- manual-regeneration
cd listings/ch21-web-server/listing-20-25
cargo run
curl http://127.0.0.1:7878
curl http://127.0.0.1:7878
curl http://127.0.0.1:7878
Третий запрос должен выдать ошибку и в окне вызова вы должны увидеть вывод, подобный следующему
...
Примите поздравления! Теперь мы завершили ящик; у нас есть основной сетевой-отдельный вычислитель, использующий объединение потоков для не согласованных ответов. Мы можем выполнить правильное завершение работы отдельного вычислителя, очистив все потоки в объединении.

...
Поскольку мы владеем `args` и будем изменять `args` в шаге повторения над ним, мы можем добавить ключевое слово `mut` в свод требований переменной `args`, чтобы сделать его изменяемым.

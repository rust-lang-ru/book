...
Когда мы писали библиотеку, мы не знали, что кто-то может добавить вид данных `SelectBox`, но наша использование `Screen` могла работать с новым видом данных и рисовать его, потому что `SelectBox` использует сущность `Draw`, это означает, что он использует способ `draw`.

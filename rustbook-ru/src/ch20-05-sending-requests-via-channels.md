...
этот возможности подойдет для решения нашей задачи. Поток работает, как цепочка задач
...
5. В потоке, образец `Worker` в круговороте получает из потока и выполняет замыкания.
...
данных, который мы будем отправлять в поток. Это код устройства, который пока ничего
не содержит:
...
<span class="caption">код 20-20: получение и выполнение заданий в круговороте, в потоке</span>
...
<span class="caption">код 20-21: добавление особенности `FnBox` для устранения ограничений
`Box<FnOnce()>`</span>

...
5. В потоке, экземпляр `Worker` в цикле получает из потока и выполняет замыкания.

Приступим к созданию потока в функции `ThreadPool::new` и содержащуюся на отправляющей
стороне экземпляр `ThreadPool`, как показано в коде 201-16. `Job` является видом
...
хранения отправленной сведений в поток, которая отправляет экземпляры `Job`</span>
...
<span class="caption">код 20-18: разделение получающую часть потока между экземплярами
...
После создания нового экземпляра `Job`, используя способ `execute` мы отправляем
задание в поток.

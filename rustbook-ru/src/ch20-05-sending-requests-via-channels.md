...
В главе 16 мы изучали потоки. потоки отличный способ общения между потоками и этой возможности подойдет для решения нашей задачи. Поток работает, как цепочка задач и функция `execute` будет отправлять задания из образца `ThreadPool` в `Worker`.
...
5. В потоке, образец `Worker` в круговороте получает из потока и использует замыкания.

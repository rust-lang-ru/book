...
Неполадка, которая у нас имеется в текущей реализации следующая - наше замыкание
...
Итак, мы хотим чтобы образец `Worker` создавал бы задачи, который `ThreadPool`
выполнял бы в потоке.

В главе 16 мы изучали потоки. потоки отличный способ общения между потоками и
этот функционал подойдет для решения нашей задачи. Поток работает, как цепочка задач
и функция `execute` будет отправлять задания из образца `ThreadPool` в `Worker`.
...
3. Новая структура `Job` будет содержать замыкание, которое мы хотим отправить в поток.
4. Способ `execute` структуры `ThreadPool` будет отправлять задание, которые мы хотим отослать.
5. В потоке, образец `Worker` в цикле получает из потока и выполняет замыкания.

Приступим к созданию потока в функции `ThreadPool::new` и содержащуюся на отправляющей
стороне образец `ThreadPool`, как показано в коде 201-16. `Job` является видом
данных, который мы будем отправлять в поток. Это код структуры, который пока ничего
не содержит:
...
В функции `ThreadPool::new` мы создаём новый поток и затем отправляем данные. Этот
код собираются (хотя и c предупреждениями).
...
Этот код 20-17 пока не будет собираться:
...
<span class="caption">код 20-17: Передача принимающего конца потока
рабочим в образец `Worker`</span>
...
<span class="caption">код 20-18: разделение получающую часть потока между образцами
используя `Arc` и `Mutex`</span>

Код будет собираться.

Теперь реализуем способ `execute` в `ThreadPool`. Мы также изменим структуру `Job`.
Вместо того, чтобы быть структурой - сделаем её псевдонимом сложного вида данных:
...
<span class="caption">код 20-19: создание вида данных `Job`, как `Box`, который
содержит замыкание, далее отправляем задание в поток</span>

После создания нового образца `Job`, используя способ `execute` мы отправляем
задание в поток.

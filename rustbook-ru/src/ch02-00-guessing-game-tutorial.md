...
Эта строка создаёт новую переменную с именем `apples` и привязывает её к значению 5. В Rust переменные неизменяемы по умолчанию, то есть, как только мы присвоим переменной значение, значение не изменится. Мы подробно обсудим эту концепцию в разделе [«Переменные и изменчивость».]<!-- ignore --> в Главе 3. Чтобы сделать переменную изменяемой, мы добавляем `mut` перед её именем:
...
Как упоминалось ранее, `read_line` помещает всё, что вводит пользователь, в строку, которую мы ему передаём, но также возвращает значение `Result`. <a data-md-type="raw_html" href="../std/result/enum.Result.html">`Result`</a><!-- ignore --> это [*перечисление*]<!-- ignore -->, часто называемый *enum*, тип, который может находиться в одном из нескольких возможных состояний. Мы называем каждое такое состояние *вариантом*.
...
Ещё многое возможно сказать о [Cargo]<!-- ignore --> и [его экосистеме]<!-- ignore -->, которые мы обсудим в Главе 14, а пока это всё, что вам нужно знать. Cargo упрощает повторное использование библиотек, поэтому Rustaceans могут создавать проекты меньшего размера, собранные из нескольких пакетов.

...
Как упоминалось ранее, функция `read_line` помещает символы  пользовательского ввода в переменную переданную в неё, но она также имеет возвращаемый тип - в этом случае это [`io::Result`]<!-- ignore -->. В стандартной библиотеке Rust имеется несколько типов с именем  `Result`: обобщённый тип<a href="../std/result/enum.Result.html" data-md-type="link">`Result`</a><!-- ignore -->, а также конкретные версии для под модулей, такие как `io::Result`.

Типы `Result` являются [*перечисление*]<!-- ignore -->, часто называемые *enums*. Перечисление имеет фиксированное множество возможных значений, которые называются *вариантами* (variants) перечисления. Глава 6 расскажет про перечисления более детально.
...
[семантическое версионирование]: ../std/io/struct.Stdin.html#method.read_line
[*перечисление*]: ../std/io/type.Result.html
[`match`]: ../std/result/enum.Result.html
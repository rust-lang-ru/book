...
Типы `Result` являются [*перечислением*]<!-- ignore -->, часто называемые *enums*. Перечисление имеет фиксированное множество возможных значений, которые называются *вариантами* (variants) перечисления. Глава 6 рассмотрит перечисления более детально.

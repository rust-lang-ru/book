...
Давайте погрузимся в программирование на Rust, работая вместе над практическим проектом! Эта глава ознакомит вас с некоторыми базовыми концепциями языка Rust, показывая их применение в реальной программе. Вы узнаете про `let`, `match`, методы, ассоциированные функции, использование внешних пакетов и многое другое! Последующие главы расскажут об этих идеях более детально. В этой главе вы будете практиковать основы.
...
Как вы уже видели в главе 1, `cargo new` генерирует программу "Hello, world!". Посмотрите файл *src/main.rs*:
...
Этот код содержит много информации, так что давайте разберём его построчно. Чтобы получить пользовательский ввод и затем вывести результат, мы должны включить библиотеку `io` (input/output) в область видимости проекта. Библиотека `io` подключается из стандартной библиотеки (которая известна как `std`):
...
Как упоминалось ранее, `read_line` помещает символы  пользовательского ввода в переменную переданную в неё, но она также имеет возвращаемый тип - в этом случае это [`io::Result`]<comment></comment>. Rust в стандартной библиотеке имеет несколько типов с именем `Result`: обобщённый тип <a data-md-type="raw_html" href="../std/result/enum.Result.html">`Result`</a><comment></comment>, а также конкретные версии для подмодулей, такие как `io::Result`.
...
Напомним, что крейт является собранием файлов исходного кода Rust. Проект который мы собираем является *бинарным крейтом*, который исполняемый. Крейт `rand` это *библиотека*, которая содержит код, предназначенный для использования в других программах.
...
После обновления реестра Cargo проверяет раздел `[dependencies]` и загружает любые крейты, которых у вас ещё нет. В этом случае, хотя мы перечислили только `rand` в качестве зависимости, но Cargo также забрал `libc` и `rand_core`, потому что работа `rand` зависит от них. После загрузки крейтов, Rust компилирует сначала их и затем компилирует проект с доступными зависимостями.
...
Когда вы *хотите* обновить крейт, Cargo предоставляет другую команду, `update`, которая проигнорирует файл *Cargo.lock* и выяснит все последние версии, которые соответствуют вашим спецификациям в файле *Cargo.toml*. Если это работает, Cargo запишет эти версии в файл *Cargo.lock*.
...
Сначала, добавим `use` строку вида: `use rand::Rng`. Типаж `Rng` определяет методы, которые реализуют генератор случайных чисел и данный типаж должен быть в области видимости, чтобы использовать его методы. Глава 10 подробно расскажет о типажах.

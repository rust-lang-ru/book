...
По умолчанию в Rust есть набор элементов, определенных в стандартной библиотеке, которые он добавляет в область видимости каждой программы. Этот набор называется *прелюдией*, и вы можете изучить его содержание [в документации стандартной библиотеки] .
...
Как упоминалось ранее, `read_line` помещает всё, что вводит пользователь, в строку, которую мы ему передаём, но также возвращает значение `Result`. <a href="../std/result/enum.Result.html" data-md-type="link">`Result`</a><!-- игнорировать --> это [*перечисление*]<!-- игнорировать -->, часто называемый *enum*, тип, который может находиться в одном из нескольких возможных состояний. Мы называем каждое такое состояние *вариантом*.

...
Как упоминалось ранее, `read_line` помещает символы  пользовательского ввода в переменную переданную в неё, но она также имеет возвращаемый тип - в этом случае это [`io::Result`]<comment></comment>. Rust в стандартной библиотеке имеет несколько типов с именем `Result`: обобщённый тип <a data-md-type="link" href="../std/result/enum.Result.html">`Result`</a><comment></comment>, а также конкретные версии для подмодулей, такие как `io::Result`.

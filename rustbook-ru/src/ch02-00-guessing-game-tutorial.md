...
Правила написания `::` в строке `::new` указывают, что `new` является сопряженной функцией вида данных `String`. *Сопряженная способ (функция)* — это способ (функция), используемая для вида данных, в данном случае `String`. Способ (функция) `new` создаёт новую пустую строку. Способ (функцию) `new` можно встретить во многих видах данных, это привычное название для способы (функции), которая создаёт новое значение какого-либо вида данных.

...
Кроме того, изложение <code>u32</code> в этом примере приложения и сравнение с <code>secret_number</code> означает, что Ржавчина сделает вывод, что <code>secret_number</code> должен быть <code>u32</code>. Итак, теперь сравнение будет между двумя значениями одного вида данных!
...
Добавление строки `break` после `You win!` заставляет программу выйти из круговорота, когда пользователь правильно угадает тайное число. Выход из круговорота также означает выход из приложения, так как круговорот является последней частью `main`.

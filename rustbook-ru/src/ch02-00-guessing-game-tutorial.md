...
Команда `run` подходит для быстрой пересборки и запуска проекта, мы так собираемся сделать в этом проекте, быстро тестируя каждое изменение, прежде чем перейти к следующему.

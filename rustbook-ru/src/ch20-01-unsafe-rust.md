...
- Вызов небезопасной способы (функции) или небезопасного способа
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-01/src/main.rs:here}}
```
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-02/src/main.rs:here}}
```
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-03/src/main.rs:here}}
```
...
С учётом всех этих опасностей, зачем тогда использовать сырые указатели? Одним из основных применений является взаимодействие с рукописью C, как вы увидите в следующем разделе ["Вызов небезопасной способы (функции) или способа"](#calling-an-unsafe-function-or-method)<!--  -->. Другой случай это создание безопасных абстракций, которые не понимает оценщик заимствований. Мы введём понятие небезопасных способов (функций) и затем рассмотрим пример безопасной абстракции, которая использует небезопасный рукопись.
...
Тела небезопасных способов (функций) являются в действительности `unsafe` разделами, поэтому для выполнения других небезопасных действий внутри небезопасной способы (функции) не нужно добавлять ещё один `unsafe` раздел.
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-04/src/main.rs:here}}
```

<span class="caption">Приложение 19-4: Использование безопасной способы (функции) <code>split_at_mut</code></span>

Эту способ (функцию) нельзя выполнить, используя только безопасную Ржавчина. Попытка выполнения могла бы выглядеть примерно как в приложении 19-5, которое не собирается. Для простоты мы используем `split_at_mut` как способ (функцию), а не как способ, и только для значений вида данных `i32`, а не обобщённого вида данных `T`.

```rust,ignore,does_not_compile
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-05/src/main.rs:here}}
```
...
```console
{{#include ../listings/ch20-advanced-features/listing-19-05/output.txt}}
```
...
Приложение 19-6 отображает, как можно использовать `unsafe` раздел, сырой указатель и вызовы небезопасных способов (функций) чтобы `split_at_mut` заработала:

```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-06/src/main.rs:here}}
```

<span class="caption">Приложение 19-6. Использование небезопасной рукописи в выполнении способы (функции) <code>split_at_mut</code></span>
...
Обратите внимание, что нам не нужно помечать итоговую способ (функцию) `split_at_mut` как `unsafe`, и мы можем вызвать эту способ (функцию) из безопасного Ржавчины. Мы создали безопасную абстракцию для небезопасной рукописи с помощью выполнения способы (функции), которая использует рукопись `unsafe` раздела безопасным образом, поскольку она создаёт только допустимые указатели из данных, к которым эта способ (функция) имеет доступ.
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-07/src/main.rs:here}}
```
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-09/src/main.rs}}
```
...
```rust
{{#rustdoc_include ../listings/ch20-advanced-features/listing-19-11/src/main.rs}}
```

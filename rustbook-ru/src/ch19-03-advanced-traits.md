...
Тогда использование сущности `OutlinePrint` для устройства `Point` будет собрана успешно и мы можем вызвать `outline_print` у образца `Point` для отображения значения обрамлённое звёздочками.

...
В качестве примера, мы хотим использовать сущность `Display` для вида данных `Vec<T>`, где "правило сироты" (orphan rule) не позволяет нам этого делать напрямую, потому что сущность `Display` и вид данных `Vec<T>` объявлены вне нашего ящика. Мы можем сделать устройство `Wrapper`, которая содержит образец данных `Vec<T>`; тогда мы можем использовать `Display` у устройства `Wrapper` и использовать значение `Vec<T>` как показано в приложении 19-23.

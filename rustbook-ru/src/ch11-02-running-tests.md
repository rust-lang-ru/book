...
Для отображения, как запустить объединение проверок, мы создадим объединение проверок для способа (функции) `add_two` function, как показано в Приложении 11-11, и постараемся выбрать какие из них запускать.
...
```console
{{#include ../listings/ch11-writing-automated-tests/output-only-04-запщущен-ignored/output.txt}}
```

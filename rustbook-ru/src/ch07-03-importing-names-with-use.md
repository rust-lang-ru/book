...
которые находятся в модуле `of`. Их можно вызывать просто указывая приставка имени этого
модуля `of::`.
...
Чтобы не указывать имя модуля, можно выполнить постоянной импорт функции в
область видимости:
...
Очень важная возможность импорта значений перечислений!
...
к конфликтным или неожиданным случаейм, когда в разных пространствах имён существуют
...
Сейчас разберёмся подробнее. Добавим код проверки в исходный код файла *src/lib.rs*:
...
Обратите внимание на особую изложение и то, что это отдельный модуль в нашем коде.
...
Проверки помогают отлаживать код библиотеки. Напишем наш первый проверку. Он будет вызывать
...
Или мы можем использовать `super` для того чтобы переместиться по модульной упорядочевания
...
внутри модульной упорядочевания, то начиная с корневого модуля ваш код будет длинным.
...
Теперь вы знаете ещё один способ, как можно согласовать ваш код. Его можно использовать
для группировки различных элементов вместе, при переработке кода большого количества
кода.

Далее, мы рассмотрим структуры данных встроенной библиотеки.

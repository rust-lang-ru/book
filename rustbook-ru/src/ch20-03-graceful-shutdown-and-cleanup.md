...
Первая ошибка находится в нашей реализации `Drop`. Ранее мы упоминали, что намеревались вызвать `take` для свойства `Option`, чтобы забрать `thread` из процесса `worker`. Следующие изменения делают это:
...
- На [crates.io](https://crates.io/) найдите крейт для работы с объединениями потоков и на его основе реализуйте подобный веб-сервер. Затем сравните его API и надёжность с реализованным нами объединением потоков.

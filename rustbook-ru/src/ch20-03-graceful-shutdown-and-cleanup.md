...
Способ `take` определён в сущности `Iterator` и ограничивает повторение самое большее первыми двумя элементами. `ThreadPool` выйдет из области видимости в конце `main` и будет запущена его использование `drop`.

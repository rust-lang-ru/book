...
Когда мы перешли с использования двух фьючерсов на три в предыдущем разделе, нам также пришлось переключиться с использования `join` на использование `join3`. Было бы раздражающе вызывать другую функцию каждый раз, когда мы меняем количество фьючерсов, которые
...
присоединиться ко всем_ из них. Функция `trpl::join_all` принимает любой вид,
...
заменить `join!` на `join_all`, как показано в Листинге 17-15.

<Listing  number="17-15" caption="Сохранение анонимных будущих событий в векторе и вызов `join_all`">
...
К сожалению, этот код не собирается. Вместо этого мы получаем такую ​​ошибку:
...
Это может быть удивительно. В конце концов, ни один из несогласованных блоков ничего не возвращает,
...
ошибок из функции run»][dyn]<!-- ignore --> в Главе 12. (Мы подробно рассмотрим
объекты черт в Главе 18.) Использование объектов черт позволяет нам рассматривать каждое
из анонимных будущих, созданных этими видами, как один и тот же вид, потому что все
...
> различные виды данных, поскольку они анонимны. Во-вторых, причина, по которой мы
...
К сожалению, этот код все еще не собирается. Фактически, мы получаем ту же самую базовую
...
сообщает нам, что первый несогласованный блок (`src/main.rs:8:23: 20:10`) не
...
мы делаем это только для того, чтобы выровнять виды данных. В конце концов, нам на самом деле _не нужно_ выделение кучи: эти будущие объекты являются локальными для этой конкретной функции.
...
Мы зашли так далеко, игнорируя то обстоятельство, что у нас могут быть разные виды данных `Output`. Например, в приложении 17-20 анонимное будущее для `a` использует `Future<Output = u32>`, анонимное будущее для `b` использует `Future<Output =
&str>`, а анонимное будущее для `c` использует `Future<Output = bool>`.
...
будущих событий с `join_all`, пока они все имеют один и тот же вид, либо мы можем иметь дело с заданным числом будущих событий с функциями `join` или макросом `join!`,
...
интересное поведение происходит в теле несогласованных блоков.
...
первым. Это потому, что реализация этой конкретной функции `race`
...
Вспомним из [Наша первая несогласованная программа][async-program]<!-- ignore -->, что в каждой точке ожидания Ржавчина дает среде выполнения возможность приостановить задачу и переключиться на
...
полезным средством для предоставления способов для различных частей программы взаимодействовать друг с другом.
...
Давайте смоделируем длительную действие. Листинг 17-22 представляет `медленную`
функцию.
...
Если вы запустите этот код, вы увидите такой вывод:
...
без запуска будущего `b` _вообще_. Давайте попробуем использовать функцию `sleep` в качестве отправной точки для отключения действий в этапе выполнения, как показано в
Листинге 17-24.
...
напрямую, используя функцию `yield_now`. В приложении 17-25 мы заменяем все эти
...
что еще делает ваша программа, потому что он предоставляет полезный средство для
структурирования отношений между различными частями программы. Это
...
измеряйте, чтобы увидеть, каковы в действительностие узкие места производительности вашего кода. Однако, если вы _видите_,
...
построить функцию `timeout` с несогласованными строительными блоками, которые у нас уже есть. Когда
...
Листинг 17-27 показывает, как мы ожидаем, что этот `timeout` будет работать с медленным будущим.
...
во вторую очередь с функциями и макросами, такими как `join`, `join_all`, `race` и т. д.

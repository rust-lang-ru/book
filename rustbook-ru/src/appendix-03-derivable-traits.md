...
Если Вам понадобилось поведение отличное от поведения при реализации через `derive`, обратитесь к [документации по стандартной библиотеке](../std/index.html)<comment></comment> чтобы узнать как вручную реализовать типаж.

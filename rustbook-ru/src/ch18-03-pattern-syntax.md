...
Ржавчина не может определить, сколько значений в упорядоченном ряде нужно пренебрегать, прежде чем сопоставить значение с `second`, и сколько следующих значений пренебрегать после этого. Этот код может означать, что мы хотим пренебрегать `2`, связать `second` с `4`, а затем пренебрегать `8`, `16` и `32`; или что мы хотим пренебрегать `2` и `4`, связать `second` с `8`, а затем пренебрегать `16` и `32`; и так далее. Имя переменной `second` не означает ничего особенного для Ржавчина, поэтому мы получаем ошибку сборщика, так как использование `..` в двух местах как здесь, является неоднозначным.

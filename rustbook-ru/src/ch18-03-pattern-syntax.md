...
Эту рукопись печатает `one`, потому что значение в `x` равно 1. Данный правила написания полезен, когда вы хотите, чтобы вашу рукопись предпринял действие, если он получает определенное значение.
...
Эту рукопись создаёт переменные `x` и `y`, которые соответствуют полям `x` и `y` из переменной `p`. В итоге переменные `x` и `y` содержат значения из устройства `p`.
...
Эту рукопись напечатает `Change the color to red 0, green 160, and blue 255`. Попробуйте изменить значение переменной `msg`, чтобы увидеть выполнение рукописи в других ветках.
...
Эту рукопись позволяет нам разбивать сложные виды на составные части, чтобы мы могли использовать нужным нас значения по отдельности.
...
Эту рукопись полностью пренебрегает значение `3`, переданное в качестве первого переменной, и выведет на печать `This code only uses the y parameter: 4`.
...
Эту рукопись напечатает `Some numbers: 2, 8, 32`, а значения 4 и 16 будут пренебрежены.
...
Эту рукопись работает правильно, потому что мы никогда не привязываем `s` к чему либо; оно не перемещается.

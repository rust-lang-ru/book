...
Listing 18-14 shows a `match` expression that separates `Point` values into three cases: points that lie directly on the `x` axis (which is true when `y =
...
<span class="caption">Listing 18-17: Using <code>_</code> in a function signature</span>
...
<span class="caption">Listing 18-18: Using an underscore within patterns that match <code>Some</code> variants when we don’t need to use the value inside the <code>Some</code></span>
...
<span class="caption">Listing 18-23: Ignoring all fields of a <code>Point</code> except for <code>x</code> by using <code>..</code></span>
...
<span class="caption">Listing 18-25: An attempt to use <code>..</code> in an ambiguous way</span>
...
<span class="caption">Listing 18-29: Using <code>@</code> to bind to a value in a pattern while also testing it</span>

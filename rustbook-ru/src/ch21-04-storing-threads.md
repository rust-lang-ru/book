...
<span class="caption">рукопись 20-13: выполнение способы (функции) `ThreadPool::new`, которая прервёт
...
`thread::JoinHandle` в качестве вида переменных в `ThreadPool`. Мы ещё не использовали способ (функцию) `with_capacity`. Она делает то же самое, что и `Vec::new` - изменяет размеры при вставке переменных. Поскольку мы создали вектор нужного размера, который нам нужен, никаких изменений размера не потребуется.
...
```
`size` имеет вид данных `u32`. Способы (функции) `Vec::with_capacity` нужно входное свойство вида данных
`usize`. У нас есть две возможности - мы можем изменить вид свойства способы (функции) или мы можем
привести вид данных `u32` к виду данных `usize`. Как вы помните, когда мы объявляли способ (функцию) `new`
мы не задумывались о виде входных данных. Задумаемся сейчас. Вид данных `usize` имеет большое значение для вектора. Давайте изменим описание способы (функции):
...
3. Опишем способ (функцию) `Worker::new`, чтобы она получала бы `id` и возвращала бы

...
Давайте вернёмся к ошибке в приложении 7-4, которая говорит, что раздел `hosting` является закрытым. Мы хотим, чтобы способ (функция) `eat_at_restaurant` из родительского раздела имела доступ к способы (функции) `add_to_waitlist` в дочернем разделе, поэтому мы помечаем раздел `hosting` ключевым словом `pub`, как показано в приложении 7-5.
...
Поскольку мы сделали общедоступным перечисление `Appetizer`, то можно использовать исходы `Soup` и `Salad` в способы (функции) `eat_at_restaurant`.

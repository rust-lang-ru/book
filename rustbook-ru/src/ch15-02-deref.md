...
Вид данных `Box<T>` в конечном итоге определяется как стопки составного ряда с одной переменной, поэтому в приложении 15-8 подобным образом определяется `MyBox<T>`. Мы также определим функцию `new`, чтобы она соответствовала функции `new`, определённой в `Box<T>`.

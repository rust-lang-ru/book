...
Ржавчина заменяет приказчик `*` вызовом способа `deref` и затем простое разыменование, поэтому нам не нужно думать о том, нужно ли нам вызывать способ `deref`. Эта функция Ржавчины позволяет писать рукопись, которая исполняется одинаково, независимо от того, есть ли у нас обычная ссылка или вид, выполняющий особенность `Deref`.

...
Как обсуждалось в разделе [“Реализация трейта для типа”.]<!--  --> в разделе главы 10, чтобы реализовать типаж, нам нужно предоставить реализации для требуемых методов типажа. `Deref`, предоставляемый стандартной библиотекой, требует от нас реализации одного метода с именем `deref`, который заимствует `self` и возвращает ссылку на внутренние данные. Листинг 15-10 содержит реализацию `Deref` для добавления к определению `MyBox`:
...
[”Использование структур кортежей без именованных полей для создания разных типов”]: ch05-01-defining-structs.html#using-tuple-structs-without-named-fields-to-create-different-types
...
Оператор `*` часто используется при проверке для подключения всего что есть в звене `tests`; мы поговорим об этом в разделе ["Как писать проверки"]<!-- ignore --> Главы 11. Оператор `*` также иногда используется как часть образца *самостоятельного подключения (prelude)*: смотрите [документацию по встроенной библиотеке](https://doc.rust-lang.org/std/prelude/index.html#other-preludes)<!-- ignore --> для получения дополнительной сведений об этом образце.

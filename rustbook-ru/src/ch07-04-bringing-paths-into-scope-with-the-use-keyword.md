...
Оператор `*` часто используется при тестировании для подключения всего что есть в модуле `tests`; мы поговорим об этом в разделе ["Как писать тесты"]<!--  --> Главы 11. Оператор `*` также иногда используется как часть шаблона <em>автоматического импорта (prelude)</em>: смотрите <a>документацию по стандартной библиотеке</a><comment></comment> для получения дополнительной информации об этом шаблоне.

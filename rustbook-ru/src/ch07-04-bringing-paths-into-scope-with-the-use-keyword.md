...
Необходимость указывать пути к способам (функциям) вызова может показаться неудобной и повторяющейся. В приложении 7-7 независимо от того, выбирали ли мы безусловный или относительный путь к способу (функции) `add_to_waitlist` , каждый раз, когда мы хотели вызвать `add_to_waitlist` , нам также приходилось указывать пути к разделам `front_of_house` и `hosting` . К счастью, мы можем это упростить: мы можем один раз создать *ярлык* для полного безусловного *пути* при помощи ключевого слова `use`, а затем указывать ссылку на ярлык(который занимает меньше знаков) везде  для обращения к содержимому, например способу(функции).

В приложении 7-11 мы подключили раздел `crate::front_of_house::hosting` в область действия способа (функции) `eat_at_restaurant`, поэтому нам достаточно только указать `hosting::add_to_waitlist` для вызова способа (функции) `add_to_waitlist` внутри `eat_at_restaurant`.
...
В приложении 7-11 вы могли бы задаться вопросом, почему мы указали  `use crate::front_of_house::hosting`, а затем вызвали `hosting::add_to_waitlist` внутри `eat_at_restaurant` вместо указания в  `use` полного пути прямо до способа (функции) `add_to_waitlist` для получения того же итога, что в приложении 7-13.
...
<span class="caption">Приложение 7-13: Добавление способа (функции) <code>add_to_waitlist</code> в область видимости с <code>use</code> не обычным способом</span>
...
Добавление `rand` в качестве дополнения в *Cargo.toml* указывает Cargo загрузить дополнение `rand` и все его зависимости из [crates.io](https://crates.io/) и сделать `rand` доступным для нашего ящика.

...
Если хотим подключить в область видимости *все* общие элементы, определённые в пути, можно указать путь за которым следует оператор глоб `*`:
...
Оператор glob часто используется при тестировании для подключения всего что есть в модуль `tests`; мы поговорим об этом в разделе [«Как писать тесты»](ch11-01-writing-tests.html#how-to-write-tests) главы 11. Оператор glob также иногда используется как часть шаблона автоматического импорта прелюдии: смотрите [стандартная библиотечная документация](../std/prelude/index.html#other-preludes) для получения дополнительной информации об этом шаблоне.

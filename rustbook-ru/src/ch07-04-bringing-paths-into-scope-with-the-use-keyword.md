...
Если мы используем несколько способов, определённых в одном дополнениее или в том же разделе, то перечисление каждой переменной в отдельной строке может занимать много вертикального пространства в файле. Например, эти два указания `use` используются в программе угадывания числа (приложение 2-4) для подключения способов из `std` в область видимости:

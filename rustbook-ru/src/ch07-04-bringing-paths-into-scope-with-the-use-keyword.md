...
Оператор glob часто используется при тестировании для подключения всего что есть в модуль `tests`; мы поговорим об этом в разделе [«Как писать тесты»](ch11-01-writing-tests.html#how-to-write-tests)<comment></comment> главы 11. Оператор `glob` также иногда используется как часть шаблона автоматического импорта прелюдии: смотрите [стандартная библиотечная документация](../std/prelude/index.html#other-preludes)<comment></comment> для получения дополнительной информации об этом шаблоне.

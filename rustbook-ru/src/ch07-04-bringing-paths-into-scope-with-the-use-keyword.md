...
В приложении 7-11 мы подключили раздел `crate::перед_дома::hosting` в область действия способа (функции) `eat_at_restaurant`, поэтому нам достаточно только указать `hosting::добавить_в_ожидание` для вызова способа (функции) `добавить_в_ожидание` внутри `eat_at_restaurant`.
...
Обратите внимание, что `use` создаёт ярлык только для той именно области, в которой объявлено `use`. В приложении 7-12 способ (функция) `eat_at_restaurant` передаётся в новый дочерний раздел с именем `customer`, область действия которого отличается от области действия указания `use`, поэтому ящик не будет собран:
...
В приложении 7-11 вы могли бы задаться вопросом, почему мы указали  `use crate::перед_дома::hosting`, а затем вызвали `hosting::добавить_в_ожидание` внутри `eat_at_restaurant` вместо указания в  `use` полного пути прямо до способа (функции) `добавить_в_ожидание` для получения того же итога, что в приложении 7-13.

...
Свойства способы (функции) также могут быть образцами данных. Рукопись в приложении 18-6 объявляет способ (функцию) с именем `foo`, которая принимает одно свойство с именем `x` вида данных `i32`, к настоящему времени это должно выглядеть знакомым.

```rust
{{#rustdoc_include ../listings/ch19-patterns-and-matching/listing-18-06/src/main.rs:here}}
```

<span class="caption">Приложение 18-6: Ярлык способы (функции) использует образцы в свойствах</span>

`x` это часть образца! Как и в случае с `let`, мы можем сопоставить составной ряд в переменных способы (функции) с образцом. Приложение 18-7 разделяет значения в составном ряде при его передачи в способ (функцию).
...
Добавляя к вышесказанному, мы можем использовать образцы в списках свойств замыкания таким же образом, как и в списках свойств способы (функции), потому что, как обсуждалось в главе 13, замыкания похожи на способы (функции).

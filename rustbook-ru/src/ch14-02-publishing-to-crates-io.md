...
- **Panics**: Сценарии, в которых документированная функция может вызывать сбой. Вызывающие функцию, которые не хотят, чтобы их программы вызвал сбойи, должны убедиться, что они не вызывают функцию в этих случаях.
...
Теперь, если мы изменим либо функцию, либо пример, так что `assert_eq!` в примере вызывает сбой, и снова запустим `cargo test`, мы увидим, что проверки документации обнаруживают, что пример и код не согласованы друг с другом!
...
<span class="caption">Рисунок 14-3: Первая страница документации для <code>art</code>, в которой перечислены звенья <code>kinds</code> и <code>utils</code></span>

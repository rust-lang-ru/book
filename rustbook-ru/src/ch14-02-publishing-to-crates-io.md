...
- **Panics**: Задумки, в которых документированная функция может вызывать сбой. Вызывающие функцию, которые не хотят, чтобы их приложения вызвал сбои, должны убедиться, что они не вызывают функцию в этих случаях.
...
Хорошей новостью является то, что если средство *не удобно* для использования из другой библиотеки, вам не нужно перестраивать внутреннее дерево: вместо этого вы можете реэкспортировать переменные, чтобы сделать открытый доступ, отличное от вашего внутреннего представления в виде дерева, используя `pub use`. Реэкспорт берет открытую переменную, функцию, способ в одном месте и делает его открытым в другом месте, как если бы он был определён в другом месте.
...
В случаях, когда имеется много вложенных разделов, реэкспорт видов данных на верхнем уровне с помощью `pub use` может существенно повысить удобство работы для людей, использующих дополнение. Ещё одно распространённое использование `pub use` - это реэкспорт определений зависимого раздела в текущем дополнениее, чтобы сделать определения этого дополнения частью открытого API вашего дополнения.

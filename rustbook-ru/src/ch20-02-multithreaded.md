...
Замечательно! Ошибка говорит о том, что нам нужен вид или звено `ThreadPool`, поэтому мы сейчас его создадим. Наша выполнение `ThreadPool` не будет зависеть от того, что делает наш сетевой-отдельный вычислитель. Итак, давайте переделаем ящик `hello` из двоичного в библиотечный, чтобы хранить там нашу выполнение `ThreadPool`. После того, как мы переключимся в библиотечный ящик, мы также сможем использовать отдельную библиотеку объединения потоков для любой подходящей работы, а не только для обслуживания сетевых-запросов.

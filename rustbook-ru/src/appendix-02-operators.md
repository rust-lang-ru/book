...
`&` | `&type`, `&mut type`, `&'a type`, `&'a mut type` | Указание что данный вид заимствуется |
...
`<type>::...` | Сопряженный элемент для вида данных, который не может быть назван прямо (например `<&T>::...`, `<[T]>::...`, etc.)
...
`T: 'a` | Обобщённый вид `T` должен существовать не меньше чем `'a` (то есть вид данных не может иметь ссылки с временем жизни меньше чем `'a`)

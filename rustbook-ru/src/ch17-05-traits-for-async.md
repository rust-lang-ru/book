...
Когда вы видите код, который использует `await`, Ржавчина собирает его под капотом в код, который вызывает `poll`. Если вы посмотрите на приложение 17-4, где мы распечатали
...
Однако если Ржавчина ссобирает его именно в этот код, каждый `await` будет
...
приостанавливает future, пока он не будет готов с `Some(message)` или `None`, когда
...
исходного кода, либо путем получения права собственности на эти данные, либо путем получения изменяемой или
...
Ржавчина, будущие состояния Ржавчина, создаваемые для несогласованных блоков, могут оказаться со ссылками на
...
объект всякий раз, когда он перемещается, но это может добавить много накладных расходов на производительность,
особенно если требуется обновление целой сети ссылок. Если бы мы могли вместо этого убедиться,
что рассматриваемая структура данных _не перемещается в памяти_, нам не пришлось бы
...
перемещается, _но данные, на которые он указывает, находятся в том же месте_, как на рисунке
...
когда вы создаете саму среду выполнения, а не для повседневного кода Ржавчина.
...
потому что ему нужно сигнализировать, есть ли еще сообщения, как это делает повторитель.

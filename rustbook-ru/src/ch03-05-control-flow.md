...
Для изучения выражения `if` создайте новый ящик под названием *branches* в папке *projects*. В файл <em>src/main.rs</em> поместите следующую рукопись:
...
Часто бывает полезно выполнить раздел рукописи более одного раза. Для этой задачи Ржавчина предоставляет несколько круговоротов, которые позволяют выполнить раздел рукописи  до конца, а затем сразу же вернуться в начало. Для опытов с круговоротами давайте создадим новый ящик под названием *loops*.

В Ржавчине есть три вида круговоротов: `loop`, `while` и `for`. Давайте попробуем каждый из них.

#### Повторный запуск рукописи с помощью `loop`
...
В качестве примера, измените рукопись файла *src/main.rs* в папке приложения *loops* на рукопись ниже:
...
```console
$ cargo run
   Сборка loops v0.1.0 (file:///projects/loops)
    Finished dev [unoptimized + debuginfo] target(s) in 0.29s
     Запщущен `target/debug/loops`
снова!
снова!
снова!
снова!
...
Вы справились! Это была большая глава: вы узнали о переменных, одиночных и составных видах данных, способ (функция)х, примечаниях, выражениях `if` и круговоротах! Для опытов работы со средствами, обсуждаемыми в этой главе, попробуйте создать приложения для выполнения следующих действий:

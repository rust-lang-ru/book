...
<<<<<<< HEAD
Обе устройства, `Post` и `DraftPost`, имеют закрытое поле `content`, в котором хранится текст сообщения дневника. Устройства больше не содержат поле `state`, потому что мы перемещаем кодирование состояния в виды устройств. Устройства `Post` будет представлять обнародованную размещение, и у неё есть способ `content`, который возвращает `content`.
=======

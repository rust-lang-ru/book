...
<span class="caption">Приложение 17-12. Определение устройства <code>Post</code> и функции <code>new</code>, которая создаёт новый образец данных <code>Post</code>, сущности <code>State</code> и устройства <code>Draft</code></span>
...
Способ `request_review` в `Draft` должен вернуть новый образец данных новоuj устройства `PendingReview`, обёрнутый в Box. Эта устройства будет представлять состояние, в котором запись ожидает проверки. Устройства `PendingReview` также использует способ `request_review`, но не выполняет никаких преобразований. Она возвращает сама себя, потому что, когда мы запрашиваем проверку записи, уже находящейся в состоянии `PendingReview`, она всё так же должна продолжать оставаться в состоянии `PendingReview`.

...
> Примечание: действительное определение макроса `vec!` в встроенной библиотеке содержит рукопись для предварительного выделения правильного размера памяти. Эту рукопись является переработкой, которую мы здесь не используем, чтобы сделать пример проще.

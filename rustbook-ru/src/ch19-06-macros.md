...
<span class="caption">Приложение 19-32: Образец данных <code>DeriveInput</code> получаемый, когда разбирается рукопись имеющий свойство макроса из приложения 19-30</span>
...
Теперь, когда у нас есть рукопись для преобразования определеного Ржавчиной рукописи из `TokenStream` в образец данных `DeriveInput`, давайте создадим рукопись выполняющая сущность `HelloMacro` у определеного вида данных, как показано в приложении 19-33.
...
Мы получаем образец данных устройства `Ident` содержащий имя (определитель) определеного вида данных с использованием `ast.ident`. Устройства в приложении 19-32 показывает, что когда мы запускаем функцию `impl_hello_macro` для рукописи из приложения 19-30, то получаемый `ident` будет иметь поле `ident` со значением `"Pancakes"`. Таким образом, переменная `name` в приложении 19-33 будет содержать образец данных устройства `Ident`, что при выводе выдаст строку `"Pancakes"`, что является именем устройства в приложении 19-30.

...
Поля этого устройства показывают, что рукопись Ржавчины, которую мы разобрали, является разделом устройства с `ident` (определителем, означающим имя) `Pancakes`. В этом устройстве есть больше полей для описания всех видов рукописи Ржавчины; проверьте [пособие `syn` о устройстве `DeriveInput`] для получения дополнительных сведений.

Вскоре мы определим функцию `impl_hello_macro`, в которой построим новый, дополнительная рукопись Ржавчины. Но прежде чем мы это сделаем, обратите внимание, что выводом для нашего выводимого (derive) макроса также является `TokenStream`. Возвращаемый `TokenStream` добавляется в рукопись, написанный пользователями макроса, поэтому, когда они соберут свой ящик, они получат дополнительную возможность, которую мы предоставляем в изменённом `TokenStream`.

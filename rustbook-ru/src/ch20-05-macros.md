...
- Похожие на способы (функции) (function-like) макросы, которые выглядят как вызовы функций, но работают с TokenStream
...
Мета программирование полезно для уменьшения размера рукописи, которую вы должны написать и поддерживать, что также является одним из предназначений функций. Однако макросы имеют некоторые дополнительные возможности, которых способы (функции) не имеют.
...
Другое важное различие между макросами и способами (функциями) заключается в том, что вы должны объявить макросы или добавить их в область видимости *прежде* чем можете вызывать их в файле, в отличии от функций, которые вы можете объявить где угодно и вызывать из любого места.
...
Чтобы начать определение процедурного макроса, поместите рукопись приложения 19-31 в ваш файл *src/lib.rs* дополнения `здравствуй_macro_derive`. Обратите внимание, что эта рукопись не собирается пока мы не добавим определение для способы (функции) `impl_здравствуй_macro`.
...
Обратите внимание, что мы разделили рукопись на способ (функцию) `здравствуй_macro_derive`, которая отвечает за связанную оценку`TokenStream` и способ (функцию) `impl_здравствуй_macro`, которая отвечает за преобразование согласно правил написания дерева: это делает написание процедурного макроса удобнее. Рукопись во внешней способы (функции) ( `здравствуй_macro_derive` в данном случае) будет одинаковой для почти любого процедурного макрос дополнения, который вы видите или создаёте. Рукопись, которую вы указываете в теле внутренней способы (функции) (в данном случае `impl_здравствуй_macro` ) будет отличаться в зависимости от цели вашего процедурного макроса.

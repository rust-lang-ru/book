...
<span class="caption">Приложение 19-29: Пример определения исполняемого макроса</span>
...
Давайте создадим дополнение с именем `здравствуй_macro`, который определяет сущность с именем `здравствуйMacro` и имеет одну с ним сопряженную способ (функцию) с именем `здравствуй_macro`. Вместо того, чтобы пользователи нашего дополнения самостоятельно выполнили сущность `здравствуйMacro` для каждого из своих видов данных, мы предоставим им процедурный макрос, чтобы они могли определять свой вид данных с помощью средства `#[derive(HelloMacro)]` и получили выполнение по умолчанию для способы (функции) `здравствуй_macro`. Выполнение по умолчанию выведет `здравствуй, Macro! My name is TypeName!`, где `TypeName` - это имя вида данных, для которого был определён эту сущность. Другими словами, мы напишем дополнение, использование которого позволит другому программисту писать рукопись показанный в приложении 19-30.
...
<span class="caption">Приложение 19-30: Рукопись, которую сможет написать пользователь нашего дополнения при использовании нашего исполняемого макроса</span>
...
Чтобы начать определение исполняемого макроса, поместите рукопись приложения 19-31 в ваш файл *src/lib.rs* дополнения `здравствуй_macro_derive`. Обратите внимание, что эта рукопись не собирается пока мы не добавим определение для способы (функции) `impl_здравствуй_macro`.
...
Обратите внимание, что мы разделили рукопись на способ (функцию) `здравствуй_macro_derive`, которая отвечает за связанную оценку`TokenStream` и способ (функцию) `impl_здравствуй_macro`, которая отвечает за преобразование согласно правил написания дерева: это делает написание исполняемого макроса удобнее. Рукопись во внешней способы (функции) ( `здравствуй_macro_derive` в данном случае) будет одинаковой для почти любого исполняемого макрос дополнения, который вы видите или создаёте. Рукопись, которую вы указываете в теле внутренней способы (функции) (в данном случае `impl_здравствуй_macro` ) будет отличаться в зависимости от цели вашего исполняемого макроса.
...
Данный свойство `#[route]` будет определён платспособом как процедурный макрос. Ярлык способы (функции) определения макроса будет выглядеть так:
...
Здесь есть два свойства вида данных `TokenStream`. Первое -  для содержимого свойства: часть `GET, "/"` . Второе - это тело переменной, к которому прикреплён свойство: в данном случае `fn index() {}` и остальная часть тела способы (функции).

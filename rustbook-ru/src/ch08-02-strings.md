...
Лучший способ работать с фрагментами строк - четко указать, нужны ли вам символы или байты. Для отдельных скалярных значений в Юникоде используйте метод `chars`.  Вызов `chars` у “Зд” выделяет и возвращает два значения типа `char`, и вы можете выполнить итерацию по результату для доступа к каждому элементу:

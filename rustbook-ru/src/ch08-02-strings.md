...
Поскольку строки используются для очень многих вещей, можно использовать множество API для строк, предоставляющих множество возможностей. Некоторые из них могут показаться избыточными, но все они занимаются своим делом!  В данном случае `String::from` и `to_string` делают одно и тоже, поэтому выбор зависит от исполнения, которое вас больше всего устраивает.

Запомните, что строки хранятся в представлении UTF-8, поэтому можно использовать любые данные, которые могут быть правильно преобразованы данные в UTF-8, как показано в приложении 8-14:
...
Все это допустимые собрания знаков для вида данных `String`, представленных в UTF-8.
...
После выполнения способа `push_str` строка `s` будет содержать значение `foobar`. В ярлыке способа `push_str` принимается переменная строковый срез, потому что нам не обязательно быть владельцем значения. Например, рукопись в приложении 8-16 показан исход, когда мы не передаем владение строкового среза `s2` другой строке - `s1` через способ `push_str`. Мы просто добавляем строковый срез `s2` к строке `s1`.
...
Во-первых, перед `s2` мы видим `&`, что означает что мы складываем *ссылку* на вторую строку с первой строкой. Это происходит из-за  свойства `s` в способе `add`: мы можем добавить только `&str` к `String`; мы не можем сложить два значения `String`. Но подождите — вид данных `&s2` это `&String`, а не `&str`, как определён второй свойство в `add`. Так почему рукопись в приложении 8-18 собирается?

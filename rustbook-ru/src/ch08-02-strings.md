...
Мы говорили о строках в главе 4, но сейчас мы рассмотрим их более подробно. Новички в Ржавчине обычно застревают на строках из-за  сочетания трёх причин: склонность сборщика Ржавчины  к выявлению возможных ошибок, более сложный вид данных чем считают многие программисты и UTF-8. Эти обстоятельства объединяются таким образом, что освоение показаться сложным, если вы пришли из других языков программирования.

Полезно обсуждать строки в среде собраний, потому что строки выполнены в виде набора байтов, дополнительно некоторые способы для обеспечения полезной возможности, когда эти байты преобразуются как писание. В этом разделе мы поговорим о действиях над видом данных `String` таких как создание, обновление и чтение, которые есть у каждого вида собраний. Мы также обсудим какими особенностями вид `String` отличается от других собраний, а именно каким образом упорядочивание в  `String` осложняется различием между тем как люди и компьютеры преобразуют данные заключённые в виде `String`.
...
<span class="caption">Приложение 8-11. Создание новой пустой </code> строки</span> вида данных <code>String

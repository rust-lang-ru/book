## Проектирование внешней оболочки объединения потоков

Давайте поговорим о том, как должен выглядеть объединение Авторы часто находят
что при попытке создать некоторый код, напив сначала клиентский внешняя оболочка можно
...
код, который вызывает функции, которые мы хотели бы иметь. Ошибки сборки будут
...
потока. Это не будет окончательным решение, т.к. существует потенциальная неполадка
...
### Реализация подобного внешней оболочки с помощью `ThreadPool`

Мы хотим, чтобы объединениепотоков работал похожим образом. В коде 20-12 заменим
...
Давайте попробуем собрать данный код. Мы получим следующие ошибки:
...
библиотека реализующая работу объединения потоков будет написана, мы сможем использовать
её в любых реализациях.

Итак, дополнение будет содержать файл *src/lib.rs*  с простыми определением структуры
...
Далее, попытайтесь теперь проверить соблюдение правил нашего кода, получил следующие
...
`ThreadPool`. Также мы знаем, что функции `new` потребуется один свойство, который
может принять знание `4` в качестве переменной. Также эта функция должна возвращать
образец структуры `ThreadPool`. Давайте реализуем такую функцию, которая будет
...
Му установили `u32` в качестве вида входящего свойства переменной `size`, т.к.
...
ошибку. Реализуем способ `execute`. Если вы помните главу 13, мы можем использовать
замыкание в качестве свойства, как в трёх различных типажах: `Fn`, `FnMut` и `FnOnce`.
...
Обратите внимание, что код собирается. Но если вы попытаетесь запустить программу
`cargo run` вы получите ошибки, как в начала нашей главы. Пока наша библиотека не
готова к использованию.

> О языках со строгими сборщиками говорят (как о Haskell  и Rust), что если
> код собирается - он работает. Очень важно понять, что это всего лишь этап, а
> не конечное решение. Наш код собирается, но он пока ещё ничего не делает. Сейчас
> наступает этап написать проверки, которые бы проверили бы соблюдение правил поведения кода.

...
Отлично! Следующим нашим действием будет выполнение функции `new` для устройства `ThreadPool`. Также мы знаем, что функции `new` потребуется одно свойство, который может принять знание `4` в качестве переменной. Также эта функция должна возвращать
образец устройства `ThreadPool`. Давайте выполняем такую функцию, которая будет иметь все эти свойства:

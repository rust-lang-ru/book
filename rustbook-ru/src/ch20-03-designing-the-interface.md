...
лучше понять как лучше выполнить отделенную вычисляемую часть. Напишите API рукописи, которое будет
внутренне выстроено таким образом, чтобы его было удобно вызывать, а затем запустите способ этого устройства, а не наоборот.
...
замыкание в качестве свойства, как в трёх различных сущностях: `Fn`, `FnMut` и `FnOnce`.
Какую же сущность нам лучше использовать? Т.к. мы должны выполнить что-то вроде

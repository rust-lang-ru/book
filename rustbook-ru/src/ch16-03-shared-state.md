...
Мы создаем два потока и меняем имена переменных, используемые во втором потоке на `handle2` и `num2`. Когда мы запускаем код в этот раз, компиляция даёт нам следующее:

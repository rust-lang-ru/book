...
Обратите внимание, что если вы используете простые числовые действия, то существуют виды данных более простые, чем `Mutex<T>`, которые предоставляет звено [`std::sync::atomic` встроенной библиотеки]<!-- ignore -->. Эти виды данных обеспечивают безопасный, одновременный, атомарный доступ к простым видам данных. Мы решили использовать `Mutex<T>` с простым видом данных в этом примере, чтобы подробнее рассмотреть, как работает `Mutex<T>`.

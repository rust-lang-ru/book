...
Этот вывод лучше: у нас теперь есть разумное сообщение об ошибке. Тем не менее, мы также имеем посторонние сведения, которые мы не хотим предоставлять нашим пользователям. Возможно, использованное средство, которое мы использовали в приложении 9-13, не является лучшим способом для использования: вызов `panic!` больше подходит для программирования сбоев, чем решения сбоев, [как обсуждалось в главе 9]<!-- ignore -->. Вместо этого мы можем использовать другое средство, о котором вы узнали в главе 9 [возвращая `Result`]<!-- ignore -->, которое указывает на успех, либо на ошибку.

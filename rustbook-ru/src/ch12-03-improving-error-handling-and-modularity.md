...
рекомендации, позволяющие разделить исходный код
...
Мы всё ещё храним данные из командной строки в векторе, наш код стал более осмысленным
...
Пример (12-6) показывает код нашего очередного улучшения. Создаём структуру `Config`.
Внутри создаём поля `query` и `filename` соответственно. Мы также изменяем содержание
и тип выходных данные метода `parse_config`. В связи с этими изменениями места хранения
...
> ситуации. Сейчас же использование метода `clone` удобно и оправдано.
...
`String::new` создаётся новый экземпляр этой структуры. Реализовав этот рефакторинг,
мы сможем вызвать переименованную функцию, написав `Config::new`. Код 12-7 демонстрирует,
...
особенностей её реализации. Приступим к улучшению этой части нашей программы!
...
среза. Если значение меньше 3, то условие проверки будет выполнено и ошибка
...
Такое сообщение об ошибке будет ясно для пользователя программы, который не знаком с
...
при вызове функции `Config::new` произошла ошибка</span>
...
удобно обрабатывать ошибки без использования макроса `panic!` (non-`panic!` error
...
к данной переменной, если она используется внутри блока.
...
вызов макроса `panic!`, но может отличаться возвращаемым значением.
Пожалуйста, проверьте работу программы после данного рефакторинга!

может отличается возвращаемым значением
...
Теперь функция `run` содержит всю логику программы, начиная с чтения файла и заканчивая
...
<span class="caption">Код программы 12-12: Внесения изменений в функцию `run`. Теперь
она возвращает значение перечисления `Result<T, E>`</span>

Обратите внимание, какие мы сделали изменения в функции. Первое, мы описали
...
на нас не накладываются ограничения на выбор типа его значения. Эта возможность даёт
...
Третье изменение - это добавление возвращаемого значения `Ok` перечисления `Result`.
В результате значение `Ok` будет содержать в себе пустой кортеж `()`.
Данное решение ещё называют "заглушкой" - синтаксическая конструкция не несущая
в себе каких-либо данных - просто индикатор, соблюдающий необходимые конвенции.
...
функцией `run`. Т.к. `run` не возвращает информации для анализа (это может быть только
информация об ошибке) её то мы и будем анализировать.
...
Давайте перенесем всё, что не относится к функции `main` из *src/main.rs* в
...
методу `new` и функции `run`. Теперь у нас есть API, функционал которого мы сможем
протестировать.
...
`Config` становится доступной в области видимости основной функции. Для вызова функции `run`

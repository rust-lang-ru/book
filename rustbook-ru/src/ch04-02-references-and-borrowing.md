...
```rust
fn calculate_length(s: &String) -> usize { // s ссылается на строку
    s.len()
} // Здесь, s выходит из области видимости. Но поскольку у s нет права собственности на то,
...
Это ограничение не приводит к возникновению эффекта гонки. Ошибки такого рода трудноуловимы
и сложны.
...
Статический анализатор кода помогает предотвратить таким образом возможные скрытые
ошибки в коде. Такие ошибки легко устранить.
...
Эта ошибка сообщает об ещё неосвещённой нами опции языка Rust - *времени жизни переменной*.
...
fn dangle() -> &String { // dangle возвращает ссылку на String
    ...
    &s // мы возвращаем ссылку на созданную строку. При выходе из области видимости

...
Сначала мы меняем `s` на `mut`. Затем мы создаем изменяемую ссылку с помощью `&mut s` у которой вызываем `change` и обновляем сигнатуру функции, чтобы принять изменяемую ссылку с помощью `some_string: &mut String`. Это даёт понять, что `change` изменит значение, которое она заимствует.

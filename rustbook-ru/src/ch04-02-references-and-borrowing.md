...
<span class="caption">Figure 4-5: A diagram of <code>&String s</code> pointing at <code>String
s1</code></span>
...
First, we had to change `s` to be `mut`. Then we had to create a mutable reference with `&mut s` and accept a mutable reference with `some_string: &mut
String`.

...
<span class="caption">Приложение 5-13: Определение способа <code>area</code> для устройства <code>Rectangle</code></span>
...
>
> В языках C и C++, используются два различных приказчика для вызова способов: используется `.`, если вызывается способ непосредственно у образца устройства и используется `->`, если вызывается способ для указателя на предмет. Другими словами, если `object` является указателем, то вызовы способа `object->something()` и ` (*object).something()` являются подобными.
>
...
Каждая устройства может иметь несколько `impl`. Например, Приложение 5-15 равнозначен рукописи, показанному в приложении 5-16, в котором каждый способ находится в своём собственном разделе `impl`.
...
Устройства позволяют создавать собственные виды, которые имеют смысл в вашей предметной области. Используя устройства, вы храните сопряженные друг с другом отрывки данных и даёте название частям данных, чтобы ваша рукопись была более понятной. Способы позволяют определить поведение, которое имеют образцы ваших устройств, а сопряженные функции позволяют привязать возможность к вашему устройству, не обращаясь к её образцу.

Но устройства — не единственный способ создавать собственные виды: давайте обратимся к перечислениям в Ржавчине, чтобы добавить ещё один средство в свой арсенал.

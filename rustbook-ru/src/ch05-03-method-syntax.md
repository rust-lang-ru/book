...
<span class="caption">Приложение 5-14: Использование ещё не написанного способа стопки <code>can_hold</code></span>
...
Мы знаем, что хотим определить способ стопки, поэтому он будет находится в разделе `impl Прямоугольник` . Имя способа стопки будет `can_hold`, и он будет принимать неизменяемое заимствование на другой `Прямоугольник` в качестве свойства. Мы можем сказать, какой это будет вид свойства, посмотрев на рукопись вызывающего способа стопки `прямоугольник1.can_hold(&rect2)`: сам способ стопки `can_hold` передаёт заимствование на стопку `&rect2` , которое является неизменяемым заимствованием образца данных `rect2` вида данных стопка `Прямоугольник`. В этом есть смысл, потому что нам нужно только получить данные из стопки `rect2` (а не вносить изменения или владеть ими, что означало бы, что нужно изменяемое заимствование), и мы хотим, чтобы `main` сохранила право собственности на образец данных `rect2`, чтобы мы могли использовать его снова после вызова способа стопки `can_hold`. Возвращаемое значение способом стопки `can_hold` имеет разумный вид данных, а его выполнение (вызов способа стопки) проверяет, являются ли ширина и высота `self` больше, чем ширина и высота другого образца данных - стопки `Прямоугольник` соответственно. Давайте добавим новый способ стопки `can_hold` в `impl` раздел из приложения 5-13, как показано в приложении 5-15.
...
<span class="caption">Приложение 5-15: Использование способа стопки <code>can_hold</code> для <code>Прямоугольник</code>, принимающего другой образец данных <code>Прямоугольник</code> в качестве свойства</span>

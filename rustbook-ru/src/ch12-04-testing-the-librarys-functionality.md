...
Наш проверка пройден, значит она работает!
...
Теперь, когда функция `search` работает и проверена, нужно вызвать `search` из нашей функции `run`. Нам нужно передать значение `config.query` и `contents`, которые `run` читает из файла, в функцию `search`. Тогда `run` выведет каждую строку, возвращаемую из `search`:
...
Чтобы завершить этот дело, мы кратко выполним пару вещей: как работать с переменными окружения и как выводить в обычный поток ошибок, обе из которых полезны при написании окно выводаных программ.

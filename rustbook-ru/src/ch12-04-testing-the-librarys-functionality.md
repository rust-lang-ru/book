...
<span class="caption">Приложение 12-15: Создание безуспешного проверки для способы (функции) <code>search</code>, которую мы хотим создать</span>
...
<span class="caption">Приложение 12-16: Определение способы (функции) <code>search</code>, достаточное, чтобы проверка собрался</span>
...
Далее мы проверяем, содержит ли текущая строка нашу искомую строку. К счастью, у строк есть полезный способ `contains`, который именно это и делает! Добавьте вызов способа `contains` в способы (функции) `search`, как показано в приложении 12-18. Обратите внимание, что это все ещё не собирается.
...
На данное мгновение мы наращиваем возможность. Чтобы заставить это собираться, нам нужно вернуть значение из тела способы (функции), как мы указали в ярлыке способы (функции).
...
На этом этапе мы могли бы рассмотреть возможности изменения выполнения способы (функции) поиска, сохраняя прохождение проверок и поддерживая имеющуюся возможность. Рукопись в способы (функции) поиска не так уж плох, но он не использует некоторые полезные способы (функции) повторителей. Вернёмся к этому примеру в [главе 13](ch13-02-iterators.html)<!-- ignore -->, где будем исследовать повторители подробно, и посмотрим как его улучшить.

#### Использование способы (функции) `search` в способы (функции) `run`

Теперь, когда способ (функция) `search` работает и проверена, нужно вызвать `search` из нашей способы (функции) `run`. Нам нужно передать значение `config.query` и `contents`, которые `run` считывает из файла, в способ (функцию) `search`. Тогда `run` выведет каждую строку, возвращаемую из `search`:
...
Мы по-прежнему используем круговорот `for` для возврата каждой строки из способы (функции) `search` и её выводе.

...
Этот тест ищет строку `"duct"`. Текст, в котором мы ищем состоит из трёх строк, только одна из которых содержит `"duct"`. Мы проверяем, что значение, возвращаемое из функции `search`, содержит только ту строку, которую мы ожидаем.
...
Метод `lines` возвращает итератор. Мы поговорим об итераторах в разделе [Глава 13]<!-- ignore -->, но вспомните, что вы видели этот способ использования итератора в [Листинге 3-5]<!-- ignore -->, где мы использовали цикл `for` с итератором, чтобы выполнить некоторый код для каждого элемента в коллекции.

...
Второй момент, функция `five` не имеет входных параметров и определяет тип возвращаемого значения, но само тело функции единственная `5` без точки с запятой, потому что является выражением, значение которого мы хотим вернуть из функции.

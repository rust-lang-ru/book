...
Способы (функции) широко распространены в рукописи Ржавчины. Вы уже познакомились с одной из самых важных функций в языке: функцией `main`, которая является точкой входа большинства программ. Вы также видели ключевое слово `fn`, позволяющее объявлять новые способы (функции).

Рукопись Ржавчины использует *змеиный регистр (snake case)* как основное исполнение для имён функций и переменных, в котором все буквы строчные, а знак подчёркивания разделяет слова. Вот приложение, содержащая пример определения способы (функции):
...
Для определения способы (функции) в Ржавчине необходимо указать `fn`, за которым следует имя способы (функции) и набор круглых скобок. Узорчатые скобки указывают сборщику, где начинается и заканчивается тело способы (функции).

Мы можем вызвать любую способ (функцию), которую мы определили ранее, введя её имя и набор скобок следом. Поскольку в программе определена `another_function`, её можно вызвать из способы (функции) `main`. Обратите внимание, что `another_function` определена *после* способы (функции) `main` в исходной рукописи; мы могли бы определить её и раньше. Ржавчине не важно, где вы определяете свои способы (функции), главное, чтобы они были определены где-то в той области видимости, которую может видеть вызывающая их рукопись.

Создадим новое исполняемое дело с названием *functions* для дальнейшего изучения функций. Поместите пример `another_function` в файл *src/main.rs* и запустите его. Вы должны увидеть следующий вывод:

```console
{{#include ../listings/ch03-common-programming-concepts/no-listing-16-functions/output.txt}}
```

Строки выполняются в том порядке, в котором они расположены в способы (функции) `main`. Сначала выводится сообщение "Здравствуй мир!", а затем вызывается `another_function`, которая также выводит сообщение.

### Свойства способы (функции)

Мы можем определить способы (функции), имеющие *свойства*, которые представляют собой особые переменные, являющиеся частью ярлыков способы (функции). Когда у способы (функции) есть свойства, необходимо предоставить ей определенные значения этих свойств. Технически определенные значения называются *переменные*, но в повседневном общении люди обычно используют слова *свойство* и *переменная* как взаимозаменяемые либо для переменных в определении способы (функции), либо для определенных значений, передаваемых при вызове способа (функции).
...
Этот пример создаёт способ (функцию) под именем `print_labeled_measurement` с двумя свойствами. Первый свойство называется `value` с видом данных `i32`. Второе свойство называется `unit_label` и имеет вид данных `char`. Затем способ (функция) выводит `value` и `unit_label`.
...
Поскольку мы вызвали способ (функцию) с `5` в качестве значения для `value` и `'h'` в качестве значения для `unit_label`, вывод приложения содержит эти значения.

### Указания и выражения

Тела функций состоят из ряда указаний, необязательно заканчивающихся выражением. До сих пор способы (функции), которые мы рассматривали, не включали завершающее выражение, но вы видели выражение как часть указания. Поскольку Ржавчина является языком, основанным на выражениях, это важное различие необходимо понимать. В других языках таких различий нет, поэтому давайте рассмотрим, что такое указания и выражения, и как их различия влияют на тела функций.
...
<span class="caption">Приложение 3-1: Объявление способы (функции) <code>main</code>, содержащей одно указание</span>
...
Способы (функции) могут возвращать значения рукописи, которая их вызывает. Мы не называем возвращаемые значения, но мы должны объявить их вид данных после стрелки ( `->` ). В Ржавчине возвращаемое значение способы (функции) является родственным значения конечного выражения в разделе тела способы (функции). Вы можете раньше выйти из способы (функции) и вернуть значение, используя ключевое слово `return` и указав значение, но большинство функций неявно возвращают последнее выражение. Вот пример такой способы (функции):
...
В рукописи способы (функции) `пять` нет вызовов функций, макросов или даже указаний  `let` — есть только одно Число `5`. Это является безусловно правильной функцией в Ржавчине. Заметьте, что возвращаемый вид данных у данной способы (функции) определён как `-> i32`. Попробуйте запустить эту рукопись. Вывод будет таким:
...
Во-вторых, у способы (функции) `пять` нет свойств и определён вид данных возвращаемого значения, но тело способы (функции) представляет собой одинокую `5` без точки с запятой, потому что это выражение, значение которого мы хотим вернуть.
...
Основное сообщение об ошибке, `несовпадение видов данных`, раскрывает ключевую неполадку этой рукописи. Определение способы (функции) `увеличить_на_единицу` сообщает, что будет возвращено `i32`, но указания не вычисляются в значение, что и выражается единичным видом данных `()`. Следовательно, ничего не возвращается, что противоречит определению способы (функции) и приводит к ошибке. В этом выводе Ржавчина выдаёт сообщение, которое, возможно, поможет исправить эту неполадку: он предлагает удалить точку с запятой для устранения ошибки.

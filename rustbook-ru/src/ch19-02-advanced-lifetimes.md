...
В главе 10 мы изучили как аннотировать ссылку с помощью МВЖ для
помощи сборщику понять какие взаимосвязи существуют между данными. В этой секции
мы рассмотрим ещё не освященные опции МВЖ:  *подвиды*, *границы* и *предметы-типажи*.

### Подвиды

Представьте, что мы хотим реализовать текстовый анализатор (parser). Для этого
необходимо создать структуру, образцы которой будут хранить ссылки на строку, которую
...
строковый срез. Структура `Parser` содержит ссылку на образец `Context`. Способ
...
Что приводит нас к основной части этого раздела: В Rust есть рычаг под названием *lifetime subtyping*,
...
В главе 10 мы обсуждали как использовать времена жизни в типажах обобщенных видов.
...
Код 19-17 отображает реализацию данного совета:
...
вид, который не содержит ссылок соответствует этим критериям, так как в собранной программе нет ссылок.
...
### Переменные времени жизни предметов-типажей

В главе 17 вы изучали предметы-типажи. Они применяются при изменяемой управления.
...
между типажами предметов и переменными времени жизни:

* по умолчанию ПВЖ для типажей-предметов `'static`.
...
В случае явного указания времени жизни типажей-предметов, например `Box<Foo>`,

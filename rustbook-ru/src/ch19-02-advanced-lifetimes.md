...
мы рассмотрим ещё не освященные опции МВЖ:  *подвиды*, *границы* и *объекты-типажи*.
...
строковый срез. Структура `Parser` содержит ссылку на экземпляр `Context`. Способ
...
быть обработана. Реальные функции такого рода должны содержать больше сведений.
...
Свойство времени жизни `'b` записывается с синтаксисом `'b`: `'a`.
...
Виды без каких-либо ссылок считаются видами со статическим временем жизни `T: 'static`.
...
В главе 17 вы изучали объекты-типажи. Они применяются при изменяемой управления.

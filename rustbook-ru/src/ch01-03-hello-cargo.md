...
> Примечание. Git - это распространенная система контроля версий. Вы можете изменить `cargo new` для использования другой системы контроля версий или без системы контроля версий, используя флаг `--vcs`. Запустите `cargo new --help`, чтобы увидеть доступные параметры.

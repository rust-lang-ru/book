...
Первая команда создает новый каталог и проект с именем *hello_cargo*. Мы назвали наш проект *hello_cargo*, и Cargo создает свои файлы в каталоге с тем же именем.
...
Cargo также предоставляет команду, называемую `cargo check`. Эта команда быстро проверяет ваш код, чтобы убедиться, что он компилируется, но не создает исполняемый файл:

...
Мы использовали подчёркивание (`_`) в качестве образца данных подстановочного знака (wildcard), который будет сопоставляться с любом значением, но не будет привязываться к этому значению. Это особенно удобно в последней ветке выражения <code>match</code>, но мы также можем использовать его в любом образце, в том числе в свойствах способы (функции), как показано в приложении 18-17.
...
<span class="caption">Приложение 18-15: Использование <code>_</code> в ярлыке способы (функции)</span>
...
В большинстве случаев, когда вам больше не нужен какое-то из свойств способы (функции), вы можете изменить её ярлык, убрав неиспользуемый свойство. Пренебрежение свойства способы (функции) может быть особенно полезно в случаях когда, например, вы используете сущность с определённой ярлыком, но тело способы (функции) в вашем выполнении не нуждается в одном из свойств. В таком случае сборщик не будет выдавать предупреждения о неиспользуемых свойствах способы (функции), как это было бы, если бы вы указали имя свойства.

...
После того, как мы создали файл *tests/common/mod.rs*, мы можем использовать его в любых файлах встроенных проверок как обычный раздел. Вот пример вызова способа (функции) `setup` из проверки `it_adds_two` в файле *tests/integration_test.rs*:
...
Обратите внимание, что  объявление `mod common;` совпадает с объявлением раздела, которое отображено в приложении 7-21. Затем в проверочном способе (функции) мы можем вызвать способ (функцию) `common::setup()`.

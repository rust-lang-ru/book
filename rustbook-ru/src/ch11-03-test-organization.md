...
Напомним, что когда мы порождали новый ящик `adder` в первом разделе этой Главы, то Cargo создал для нас рукопись ниже:
...
Каждый файл в папке `tests` представляет собой отдельный дополнение, поэтому нам нужно подключить нашу библиотеку в область видимости каждого проверочного дополнения. По этой причине мы добавляем `use adder` в верхней части рукописи, что не нужно нам делать в состоящих из разделов проверках.
...
Первый раздел для состоящих из разделов проверок такой же, как мы видели: одна строка для каждого состоящего из разделов проверки (один с именем `внутреннее`, который мы добавили в приложении 11-12), а затем сводная строка для состоящих из разделов проверок.

Раздел встроенных проверок начинается со строки `Запщущен tests/integration_test.rs`. Далее идёт строка для каждого проверочного способа (функции) в этой встроенной проверке и итоговая строка для итогов встроенной проверки непосредственно перед началом раздела `Doc-tests adder`.
...
Мы всё ещё можем запустить определённую способ (функцию) во встроенных проверках, указав имя проверка способа (функции) в качестве переменной в `cargo test`. Чтобы запустить все проверки в определенном файле встроенных проверок, используйте переменная `--test` сопровождаемый именем файла у приказы `cargo test`:
...
Различное поведение файлов в папке *tests* наиболее заметно, когда у вас есть набор вспомогательных способов (функций), которые будут полезны в нескольких встроенных проверочных файлах. Представим, что вы пытаетесь выполнить действия, описанные в разделе [«Разделение разделов в разные файлы»](ch07-05-separating-modules-into-different-files.html)<!--  --> Главы 7, чтобы извлечь их в общее раздел. Например, вы создали файл *tests/common.rs* и поместили в него способ (функцию) `setup`, содержащую некоторую рукопись, которая вы будете вызывать из разных проверочных способов (функций) в нескольких проверочных файлах
...
Когда мы снова запустим проверки, мы увидим новый раздел в итогах проверок для файла *common.rs*, хотя этот файл не содержит никаких проверочных способов (функций), более того, мы даже не вызывали способ (функцию) `setup` откуда либо:

```console
{{#include ../listings/ch11-writing-automated-tests/no-listing-12-shared-test-code-problem/output.txt}}
```

Упоминание файла `common` и появление в итогах выполнения проверок сообщения вида данных `запщущен 0 tests` - это не то, чего мы хотели. Мы только хотели выделить некоторую общую рукопись, которая будет использоваться другими файлами встроенных проверок.
...
Давайте объединим  знания, полученные в этой и предыдущей главах, чтобы поработать над ящиком!

...
Способ (функция) `area` должна вычислять площадь одного прямоугольника, но способ (функция), которую мы написали, имеет два свойства, и нигде в нашей программе не ясно, что эти свойства взаимосвязаны. Было бы более удобным для чтения и управляемым объединять ширину и высоту вместе. В разделе [«составные ряды»]<!-- ignore --> Главы 3 мы уже обсуждали один из способов сделать это — использовать составные ряды.
...
<span class="caption">Приложение 5-11: Попытка вывести значения образца данных <code>Прямоугольник</code></span>
...
Теперь при запуске приложения мы не получим ошибок и увидим следующий вывод:

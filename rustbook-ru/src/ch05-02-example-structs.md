...
Здесь мы определили стопку и дали ей имя `Rectangle`. Внутри узорчатых скобок определили поля как `width` и `height`, оба — вида данных `u32`. Затем в `main` создали определенный образец данных `Rectangle` с шириной в `30` и высотой в `50` единиц.

Наша функция `area` теперь определена с одним свойством, названным `rectangle`, чей вид данных является неизменяемым заимствованием стопки `Rectangle`. Как упоминалось в главе 4, необходимо заимствовать стопку, а не передавать её во владение. Таким образом функция `main` сохраняет `rect1` в собственности и может использовать её дальше. По этой причине мы и используем `&` в ярлыке и в месте вызова функции.
...
<span class="caption">Приложение 5-12: добавление свойства для вывода сущности <code>Debug</code> и выводе образца данных <code>Rectangle</code> с отладочным изменением
</span>
...
`Debug` вида данных `Rectangle`. Макрос `dbg!` может быть очень полезен, когда вы пытаетесь понять, что делает ваша рукопись!

...
Давайте попробуем! Вызов макроса `println!` теперь будет выглядеть так `println!("rect1 is {:?}", rect1);`. Ввод определетеля `:?` внутри узорчатых скобок говорит макросу `println!`, что мы хотим использовать другой вид вывода, известный как `Debug`. Особенность `Debug` позволяет выводить устройство способом, удобным для разработчиков, чтобы видеть значение во время отладки рукописи.
...
Ржавчина *выполняет* возможность для выводе отладочной сведений, но *не включает (не выводит) её по умолчанию*. Мы должны явно включить эту возможность для нашей устройства. Чтобы это сделать, добавляем внешний свойство `#[derive(Debug)]` сразу перед определением устройства, как показано в приложении 5-12.
...
<span class="caption">Приложение 5-12: добавление свойства для вывода особенности <code>Debug</code> и выводе образца <code>Rectangle</code> с отладочным изменением
</span>

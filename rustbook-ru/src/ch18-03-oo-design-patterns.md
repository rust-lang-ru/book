...
У нас все ещё есть способ (функция) `Post::new`, но вместо возврата образца `Post` она возвращает образец данных `DraftPost`. Поскольку поле `content` является закрытым и нет никаких функций, которые возвращают `Post`, просто так создать образец данных `Post` уже невозможно.

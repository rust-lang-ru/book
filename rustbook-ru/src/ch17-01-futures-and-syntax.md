...
можно прерывать и возобновлять. Внутри несогласованного раздела или несогласованной функции вы можете
использовать ключевое слово `await`, чтобы _await фьючерсной функции_ (то есть ждать, пока она станет
готовой). Любая точка, где вы ожидаете future внутри несогласованного раздела или функции, является
потенциальным местом для остановки и возобновления этого несогласованного раздела или функции.
...
неповторимый безымянный вид данных, который использует сущность `Future`. Когда Ржавчина видит функцию, помеченную `async`, он собирает ее в ненесогласованную функцию,
...
- Вся рукопись, вызываемая в теле исходной функции, заключена в
...
- Новое тело функции - это раздел `async move` из-за того, как оно использует
...
К сожалению, эта рукопись не собирается. Единственное место, где мы можем использовать ключевое слово `await`, — это несогласованные функции или блоки, а Ржавчина не позволит нам обозначить
...
вызов несогласованной функции, поэтому вместо этого мы передадим раздел `async` и явно
...
> которую вы чаще будете встречать в существующей рукописи Ржавчины. Функция `select`

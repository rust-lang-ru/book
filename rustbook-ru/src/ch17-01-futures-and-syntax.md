...
### Определение способа (функции) page_title
...
цепочки способов намного более удобными для работы. В итоге мы можем изменить тело `page_title`, чтобы связать вызовы способов (функций) `trpl::get` и `содержимое` вместе,
...
`async fn page_title` в приложении 17-1, равноценно ненесогласованного способа (функции),

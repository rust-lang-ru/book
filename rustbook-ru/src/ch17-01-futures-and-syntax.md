...
Создайте новый двоичный проект с именем `здравствуй-async` и добавьте дополнение `trpl` в качестве зависимости:
...
Функция `main` приложения может _инициализировать_ runtime, но она не является runtime
...
функцией в приложении 17-3. Если бы `main` была несогласованной функцией, то что-то еще должно было бы управлять конечным автоматом для любого будущего, возвращаемого `main`, но `main` — это отправная точка для приложения! Вместо этого мы вызвали функцию `trpl::run`

...
*Cрезы* (slices) позволяют ссылаться на непрерывную последовательность элементов в коллекции, а не на всю коллекцию. Срез является своего рода ссылкой, поэтому он не забирает во владение данные.
...
Мы изучим итераторы более детально в [Главе 13][ch13]<!-- ignore -->. Сейчас, достаточно понять, что метод `iter` при каждом вызове возвращает следующий элемент коллекции, а метод `enumerate` оборачивает результаты работы метода `iter` и возвращает каждый элемент упакованным в кортеж. Первый элемент этого кортежа возвращён из `enumerate` и является индексом, а второй элемент - ссылка на элемент коллекции которую предоставил метод `iter`. Такой способ перебора элементов массива является более удобным - не надо считать индекс самостоятельно.

Так как метод `enumerate` возвращает кортеж, мы можем использовать шаблон деструктуризации кортежа. Подробнее о шаблонах мы поговорим в [Главе 6][ch6]<!-- ignore -->. Так в цикле `for`, мы указываем шаблон `(i, &item)` который распакует значения кортежа в `i` для хранения индекса из кортежа и в `&item` который сразу же возьмёт по ссылке байт символа из кортежа. Мы используем `&` в шаблоне по причине того, что метод  `.iter().enumerate()` возвращает нам ссылку на элемент.

...
Инструмент rustfix устанавливается вместе с Rust и может автоматически исправлять предупреждения компилятора с одним единственным способом исправления проблемы. Вероятно, этот способ подойдёт и вам. Наверняка, вы уже видели предупреждения компилятора. Например, рассмотрим этот код:

...
В простейшем случае в Ржавчине проверка - это функция, определенная свойством `test`. Свойства представляют собой метаданные о отрывках рукописи Ржавчины; один из примеров свойство `derive`, который мы использовали со устройствами в главе 5. Чтобы превратить функцию в проверяющую функцию добавьте `#[test]`  в строку перед `fn`.  Когда вы запускаете проверки приказом `cargo test`, Ржавчина создаёт двоичный звено выполняющий функции определеные свойством test и сообщающий о том, успешно или нет прошла каждая проверяющая функция.

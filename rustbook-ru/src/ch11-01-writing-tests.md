...
Проверки - это способы (функции) Ржавчина, которые проверяют, что не проверочная рукопись работает ожидаемым образом. Содержимое проверочных способов (функций) обычно выполняет три следующих действия:
...
В простейшем случае в Ржавчине проверка - это способ (функция), определенная свойством `test`. Свойства представляют собой метаданные о отрывках рукописи Ржавчины; один из примеров средство `derive`, который мы использовали со стопками в главе 5. Чтобы превратить способ (функцию) в проверяющую способ (функцию) добавьте `#[test]`  в строку перед `fn`.  Когда вы запускаете проверки приказом `cargo test`, Ржавчина создаёт исполняемое раздел выполняющее способы (функции) определенные свойством test и сообщающий о том, успешно или нет прошла каждая проверяющая способ (функция).

Когда мы создаём новый ящик библиотеки с помощью Cargo, то в нём самостоятельно порождается проверочный раздел с проверку-функцией для нас. Это раздел даст вам образец данных для написания ваших проверок, так что вам не нужно искать точный вид данных и правила написания проверочных способов (функций) каждый раз, когда вы начинаете новый ящик. Вы можете добавить столько дополнительных проверочных способов (функций) и столько проверочных разделов, сколько захотите!
...
Давайте создадим новый библиотечный ящик под названием `adder`, который складывает два числа:
...
<span class="caption">Приложение 11-1: Проверочный раздел и способ (функция), созданные самостоятельно с помощью <code>cargo new</code></span>

Сейчас давайте пренебрегаем первые две строчки рукописи и сосредоточимся на способе (функции). Обратите внимание на правила написания изложении `#[test]`: это свойство указывает, что это проверочная способ (функция), поэтому запускающая проверка знает, что эту способ (функцию) следует рассматривать как проверочную. У нас также могут быть не проверяемые способы (функции) в разделе `tests`, которые помогут настроить общие задумки или выполнить общие действия, поэтому нам всегда нужно указывать, какие способы (функции) являются проверками.

В теле способа (функции) проверки используется макрос `assert_eq!`, чтобы утверждать, что `result`, который содержит итог сложения 2 и 2, равен 4. Это утверждение служит примером вида для обычной проверки. Давайте запустим его, чтобы убедиться, что эта проверка пройдена.
...
Снова выполним приказ `cargo test`. Вывод показывает наименование нашей проверку-способа (функции) - `exploration` вместо `it_works`:

```console
{{#include ../listings/ch11-writing-automated-tests/no-listing-01-changing-test-name/output.txt}}
```

Добавим ещё один проверка, но в этот раз целенаправленно сделаем так, чтобы эта новая проверка не отработала! Проверка терпит неудачу, когда что-то вызывает сбой в проверяемом способе (функции). Каждая проверка запускается в новом потоке и когда главный поток видит, что проверочный поток упал, то помечает проверка как завершившийся со сбоем. Мы говорили о простейшем способе вызвать сбой в главе 9, используя для этого известный макрос `panic!`. Введём рукопись проверку-способа (функции) `another`, как в файле *src/lib.rs* из приложения 11-3.
...
Вместо `ok`, строка `test tests::another` сообщает `FAILED`. Два новых раздела появились между отдельными итогами и сводкой: в первом отображается подробная причина каждого сбоя проверки. В данном случае проверка `another` не сработала, потому что `завершил работу из-за сбоя 'Make this test fail'`, произошло в строке 10 файла *src/lib.rs*. В следующем разделе перечисляют имена всех не пройденных проверок, что удобно, когда есть много проверок и много подробных итогов неудачных проверок. Мы можем использовать имя не пройденной проверки для его дальнейшей отладки; мы больше поговорим о способах запуска проверок в разделе ["Управление хода выполнения проверок"](ch11-02-запщущен-tests.html#controlling-how-tests-are-run)<!-- ignore -->.
...
В главе 5, приложении 5-15, мы использовали вид данных стопка `Прямоугольник`(прямоугольник) и способ `может_удержать`, который повторён в приложении 11-5. Давайте поместим эту рукопись в файл *src/lib.rs* и напишем несколько проверок для него используя макрос `assert!`.
...
<span class="caption">Приложение 11-5: Использование стопки <code>Прямоугольник</code> и её способа может_удержать из Главы 5</span>

Способ `может_удержать` возвращает истину или ложь (разумное значение), это означает, что он является наилучшим исходом использования в макросе `assert!`. В приложении 11-6 мы пишем проверка, которая использует способ `может_удержать` путём создания образца данных `Прямоугольник` шириной 8 и высотой 7 и убеждаемся, что он может содержать другой образец данных `Прямоугольник` имеющий ширину 5 и высоту 1.
...
<span class="caption">Приложение 11-6: Проверка для способа <code>может_удержать</code>, который проверяет что больший прямоугольник действительно может содержать меньший</span>

Также, в разделе `tests` обратите внимание на новую добавленную строку `use super::*;`. Раздел `tests` является обычным и подчиняется тем же правилам видимости, которые мы обсуждали в главе 7 ["Пути для ссылки на переменную внутри дерева раздела"]<!--  -->. Так как это раздел `tests` является внутренним, нужно подключить проверяемый рукопись из внешнего раздела в область видимости внутреннего раздела с проверками. Для этого используется вездесущеее подключение, так что все что определено во внешнем разделе становится доступным внутри `tests` раздела.

Мы назвали нашу проверку `наибольщее_может_удержать_наименьшее` и создали два нужных образца данных `Прямоугольник`. Затем вызвали макрос `assert!` и передали итог вызова `наибольшее.может_удержать(&наименьшее)` в него. Это выражение должно возвращать `true`, поэтому наша проверка должна пройти. Давайте выясним!
...
Поскольку правильный итог способа (функции) `может_удержать` в этом случае `false`, то мы должны инвертировать этот итог, прежде чем передадим его в `assert!` макро. Как итог, наша проверка пройдёт, если `может_удержать` вернёт `false`:

```console
{{#include ../listings/ch11-writing-automated-tests/no-listing-02-adding-another-rectangle-test/output.txt}}
```

Два проверки работают. Теперь проверим, как отреагируют проверки, если мы добавим ошибку в рукопись. Давайте изменим использование способа `может_удержать` заменив одно из разумных выражений знак сравнения с "больше чем" на противоположный "меньше чем" при сравнении ширины:
...
Наши проверки нашли ошибку! Так как в проверке `наибольшее.ширина` равно 8 и `наименьшее.ширина` равно 5 сравнение ширины в способе `может_удержать` возвращает итог `false`, поскольку Число 8 не меньше чем 5.
...
В приложении 11-7, мы напишем способ (функцию) `добвить_второе`, которая прибавляет к входному свойству `2` и возвращает значение. Затем, проверим эту способ (функцию) с помощью макроса `assert_eq!`:
...
<span class="caption">Приложение 11-7: Проверка способа (функции) <code>добвить_второе</code> с помощью макроса <code>assert_eq!</code></span>
...
Первая переменная, которую мы передаём в макрос `assert_eq!` Число `4` чей итог вызова равен `добвить_второе(2)` . Строка для этой проверки - `test tests::это_добавляет_второе ... успешно` , а писание `ok` означает, что наша проверка пройдена!

Давайте введём ошибку в рукопись, чтобы увидеть, как она выглядит, когда проверка, которая использует `assert_eq!` завершается ошибкой. Измените выполнение способа (функции) `добвить_второе`, чтобы добавлять `3`:
...
Наша проверка нашла ошибку! Проверка `это_добавляет_второе` не выполнился, отображается сообщение `assertion failed: `(left == right)`` и показывает, что `left` было `4`, а `right` было `5`. Это сообщение полезно и помогает начать отладку: это означает `left` переменная `assert_eq!` имел значение `4`, но <code>right</code> переменная для вызова <code>добвить_второе(2)</code> был со значением <code>5</code>.

Обратите внимание, что в некоторых языках (таких как Java) в библиотеках рукописи для проверки принято именовать входные свойства проверочных способов (функций) как "ожидаемое" (`expected`) и "действительное" (`actual`). В Ржавчине приняты следующие обозначения `left` и `right` соответственно, а порядок в котором определяются ожидаемое значение и производимое проверяемым рукописью значение не имеют значения. Мы могли бы написать выражение в проверке как `assert_eq!(добвить_второе(2), 4)`, что приведёт к отображаемому сообщению об ошибке `assertion failed: `(left == right)``, слева `left` было бы <code>5</code>, а справа <code>right</code> было бы <code>4</code>.

Макрос `assert_ne!` сработает успешно, если входные свойства не равны друг другу и завершится с ошибкой, если значения равны. Этот макрос наиболее полезен в тех случаях, когда мы не знаем заранее, каким значение *будет*, но знаем точно, каким оно *не может* быть. К примеру, если проверяется способ (функция), которая обязательно изменяет входные данные определённым образом, но способ изменения входного свойства зависит от дня недели, в котором запускаются проверки, что лучший способ проверить правильность работы такого способа (функции) - это сравнить и убедиться, что выходное значение способа (функции) не должно быть равным входному значению.
...
Например, есть способ (функция), которая приветствует человека по имени и мы хотим проверять эту способ (функцию). Мы хотим чтобы передаваемое ей имя выводилось в окно вывода:
...
Выполняем это с помощью другого свойства проверку-способа (функции) `#[should_panic]`. Это свойство сообщает устройству проверки, что проверка проходит, когда способ порождает ошибку. Если ошибка не порождается - проверка считается не пройденным.
...
Свойство `#[should_panic]` следует после `#[test]` и до объявления проверочного способа (функции). Посмотрим на вывод итога, когда проверка проходит:
...
как  `#[should_panic]`. Ошибка, которую мы получили во время выполнения означает, что рукопись в проверяющего способа (функции) не вызвала сбоя и завершения приложения.
...
Эта проверка пройдёт, потому что значение, которое мы поместили для `should_panic` в свойство свойства `expected` является подстрокой сообщения, с которым способ (функция) `Guess::new` вызывает сбой. Мы могли бы указать полное, ожидаемое сообщение для сбоя, в этом случае это будет `Догадка значение должно быть меньше или равно 100, got 200`. То что вы выберите для указания как ожидаемого свойства у `should_panic` зависит от того, какая часть сообщения о сбое неповторима или динамична, насколько вы хотите, чтобы ваш проверка был точным. В этом случае достаточно подстроки из сообщения сбоя, чтобы обеспечить выполнение рукописи в проверочного способа (функции) `else if значение > 100` .

Чтобы увидеть, что происходит, когда проверка `should_panic` неуспешно завершается с сообщением `expected`, давайте снова внесём ошибку в нашу рукопись, поменяв местами `if значение < 1` и `else if значение > 100` разделы:
...
Способ (функция) `it_works` теперь имеет возвращаемый вид данных `Result<(), String>`. В теле способа (функции), вместо вызова макроса `assert_eq!`, мы возвращаем `Ok(())` когда проверка успешно выполнен и `Err` со `String` внутри, когда проверка не проходит.
...
Вы не можете использовать изложение `#[should_panic]` в проверках, использующих `Result<T, E>`. Чтобы утверждать, что действие возвращает исход `Err`, *не* используйте приказчик вопросительного знака для значения `Result<T, E>`. Вместо этого используйте `assert!(значение.is_err())`.
...
[“Пренебрежение некоторых проверок, если их целенаправленно не запрашивать”]: ch11-02-запщущен-tests.html#ignoring-some-tests-unless-specifically-requested
[“Запуск подмножества проверок по имени”]: ch11-02-запщущен-tests.html#запщущен-a-subset-of-tests-by-name
...
["Пути для ссылки на переменную внутри дерева раздела"]: ch07-03-paths-for-referring-to-an-item-in-the-module-tree.html
...
Проверки - это функции Ржавчина, которые проверяют, что не проверочная рукопись работает ожидаемым образом. Содержимое проверочных функций обычно выполняет следующие три действия:
...
Итоговая строка отображается в конце: общий итог нашего проверки `FAILED`. У нас один проверка пройден и один проверка завершена со сбоем.
...
Мы назвали нашу проверку `larger_can_hold_smaller` и создали два нужных образца `Rectangle`. Затем вызвали макрос `assert!` и передали итог вызова `larger.can_hold(&smaller)` в него. Это выражение должно возвращать `true`, поэтому наша проверка должна пройти. Давайте выясним!

...
Первая переменная, которую мы передаём в макрос `assert_eq!` число `4` чей итог вызова равен `add_two(2)` . Строка для этой проверки - `test tests::it_adds_two ... ok` , а писание `ok` означает, что наш проверка пройдена!

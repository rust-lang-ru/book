...
Поскольку правильный итог способа (функции) `can_hold` в этом случае `false`, то мы должны инвертировать этот итог, прежде чем передадим его в `assert!` макро. Как итог, наша проверка пройдёт, если `can_hold` вернёт `false`:
...
<span class="caption">Приложение 11-7: Проверка способа (функции) <code>add_two</code> с помощью макроса <code>assert_eq!</code></span>
...
Давайте введём ошибку в рукопись, чтобы увидеть, как она выглядит, когда проверка, которая использует `assert_eq!` завершается ошибкой. Измените выполнение способа (функции) `add_two`, чтобы добавлять `3`:

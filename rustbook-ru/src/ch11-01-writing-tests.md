...
В простейшем случае в Ржавчина проверка - это функция, определенная свойством `test`. Свойства представляют собой метаданные о фрагментах кода Rust; один из примеров свойство `derive`, который мы использовали со структурами в главе 5. Чтобы превратить функцию в проверяющую функцию добавьте `#[test]`  в строку перед `fn`.  Когда вы запускаете проверки приказом `cargo test`, Ржавчина создаёт двоичный модуль выполняющий функции определеные свойством test и сообщающий о том, успешно или нет прошла каждая проверяющая функция.
...
<span class="caption">Приложение 11-1: Проверочный модуль и функция, созданные самостоятельно с помощью <code>cargo new</code></span>
...
Реализуем это с помощью другого свойства проверку-функции `#[should_panic]`. Этот свойство сообщает системе проверки, что проверка проходит, когда способ генерирует ошибку. Если ошибка не генерируется - проверка считается не пройденным.
...
["Примечания документации как проверки"]: ch14-02-publishing-to-crates-io.html#documentation-comments-as-tests

...
используя функцию `trpl::stream_from_iter`. Затем мы циклически перебираем элементы в потоке
...
Собрав все эти части вместе, этот код работает так, как мы хотим! Более того, теперь, когда у нас есть `StreamExt` в области видимости, мы можем использовать все его служебные
...
Сначала мы создаем функцию `get_messages`, которая возвращает `impl Stream<Item
...
Когда мы запускаем этот код, мы получаем именно те итоги, которые и ожидали:
...
нужно использовать несогласованность. Однако мы не можем сделать сам `get_messages` несогласованной функцией, потому что тогда мы вернем `Future<Output = Stream<Item = String>>`
вместо `Stream<Item = String>>`. Вызывающему пришлось бы ожидать
...
Вместо этого мы оставляем `get_messages` как обычную функцию, которая возвращает поток,
...
Сначала давайте создадим еще один поток, который будет выдавать элемент каждую миллисекунду, если мы позволим ему работать напрямую. Для простоты мы можем использовать функцию `sleep` для отправки
...
возвращаемый вид будет `impl Stream<Item = u32>`, и мы можем вызвать функцию
...
Теперь, вернувшись в несогласованный блок нашей основной функции, мы можем попытаться объединить потоки `messages` и `intervals`, как показано в приложении 17-37.
...
разные виды данных. Поток `messages` имеет вид `Timeout<impl Stream<Item =
String>>`, где `Timeout` — это вид, который использует `Stream` для вызова `timeout`. Поток `intervals` имеет вид `impl Stream<Item = u32>`. Чтобы объединить эти два потока, нам нужно преобразовать один из них в соответствии с другим. Мы

...
<!-- ignore this test because otherwise it creates здравствуй.txt which causes other
tests to fail lol -->
...
>
> fn main() {
>     let greeting_file = File::open("здравствуй.txt").unwrap_or_else(|error| {
...
```text
thread 'main' panicked at 'здравствуй.txt должен быть включен в этот ящик: Os {
code: 2, kind: NotFound, сообщение: "No such file or directory" }',
src/main.rs:5:10
```
...
Вид данных `Box<dyn Error>` является *сущность-предметом*, о котором мы поговорим в разделе ["Использование сущность-предметов, допускающих значения разных видов данных"][Сущность-предмет]<!-- ignore --> в главе 17. Пока что вы можете считать, что `Box<dyn Error>` означает "любой вид ошибки". Использование `?` для значения `Result` в способе (функции) `main` с видом ошибки `Box<dyn Error>` разрешено, так как позволяет вернуть любое значение `Err` раньше времени. Даже если тело этого способа (функции) `main` будет возвращать только ошибки вида данных `std::io::Error`, указав `Box<dyn Error>`, эта ярлык останется правильным, даже если в тело `main` будет добавлена рукопись, возвращающая другие ошибки.

...
Функция `main` может возвращать любые виды, выполняющие [особенность `std::process::Termination`]<!-- ignore -->, в которых имеется функция `report`, возвращающая `ExitCode`. Обратитесь к пособию встроенной библиотеки за дополнительными сведениями о порядке выполнения особенности `Termination` для ваших собственных видов.

...
<span class="caption">Листинг 9-8: Цепочка вызовов методов после <code>?</code> оператора</span>
...
`main` функция может возвращать любые типы, реализующие [`std::process::Termination` .]<!-- игнорировать -->. На момент написания этой статьи `Termination` был нестабильной функцией, доступной только в Nightly Rust, поэтому вы пока не можете реализовать ее для своих типов в Stable Rust, но, возможно, когда-нибудь сможете!

...
Напомним из ["Обработка потенциального сбоя с помощью типа `Result`"](ch02-00-guessing-game-tutorial.html#handling-potential-failure-with-the-result-type)<comment> <!----> </comment> главы 2, перечисление `Result`, имеющее два варианта, `Ok` и `Err`, определено следующим образом:
...
Тип `Box<dyn Error>` называется типаж объектом, о котором мы поговорим в разделе ["Использование типаж объектов, которые допускают значения различных типов"](ch17-02-trait-objects.html#using-trait-objects-that-allow-for-values-of-different-types)<comment> <comment></comment> </comment> главы 17. А пока вы можете читать обозначение `Box<dyn Error>` как "любая ошибка". Использование `?` в `main` функции с этим  возвращаемым типом также разрешено.

...
В случае успеха `File::open` значением переменной `greeting_file_result` будет образец `Ok`, содержащий указатель файла. В случае неудачи значение в переменной `greeting_file_result` будет образцом `Err`, содержащим дополнительные сведения о том, какая именно ошибка произошла.
...
Другой способ, похожий на `unwrap`, это `expect`, позволяющий указать сообщение об ошибке для макроса `panic!`. Использование `expect` вместо `unwrap` с предоставлением хорошего сообщения об ошибке выражает ваше намерение и делает более простым отслеживание источника сбоя. Правила написания способа `expect` выглядят так:
...
Эта ошибка указывает на то, что приказчик `?` разрешено использовать только в функции, которая возвращает `Result`, `Option` или другой вид, использующий `FromResidual`.
...
Вид `Box<dyn Error>` является *сущность-предметом*, о котором мы поговорим в разделе ["Использование сущность-предметов, допускающих значения разных видов"]<!-- ignore --> в главе 17. Пока что вы можете считать, что `Box<dyn Error>` означает "любой вид ошибки". Использование `?` для значения `Result` в функции `main` с видом ошибки `Box<dyn Error>` разрешено, так как позволяет вернуть любое значение `Err` раньше времени. Даже если тело этой функции `main` будет возвращать только ошибки вида `std::io::Error`, указав `Box<dyn Error>`, эта ярлык останется правильной, даже если в тело `main` будет добавлен рукопись, возвращающая другие ошибки.
...
Функция `main` может возвращать любые виды данных, использующие [сущность `std::process::Termination`]<!-- ignore -->, в которых имеется функция `report`, возвращающая `ExitCode`. Обратитесь к пособию встроенной библиотеки за дополнительными сведениями о порядке использования сущности `Termination` для ваших собственных видов.

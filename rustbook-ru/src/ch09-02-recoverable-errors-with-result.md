...
Тип `Box<dyn Error>` называется типаж объектом, о котором мы поговорим в разделе ["Использование типаж объектов, которые допускают значения различных типов"]<!-- ignore --> Главы 17. А пока вы можете читать обозначение `Box<dyn Error>` как "любая ошибка". Использование `?` в `main` функции с этим  возвращаемым типом также разрешено.

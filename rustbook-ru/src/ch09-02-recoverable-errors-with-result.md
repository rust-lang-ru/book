...
Когда функция `main` возвращает `Result<(), E>`: выполнение программы завершится со значением `0`, если `main` вернёт `Ok(())`; завершится с ненулевым значением, если `main` вернёт значение `Err`. Исполняемые программы, написанные на языке C, возвращают целые числа при выходе - программы, которые завершаются успешно, возвращают целое число `0`, а программы завершившиеся ошибкой возвращают любое целое число, отличное от `0`. Rust также возвращает целые числа от исполняемых программ, чтобы быть совместимым с этим соглашением.

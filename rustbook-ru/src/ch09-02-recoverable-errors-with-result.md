...
Напомним из ["Обработка потенциального сбоя с помощью типа `Result`"](ch02-00-guessing-game-tutorial.html#handling-potential-failure-with-the-result-type)<comment> <comment></comment> </comment> главы 2, перечисление `Result`, имеющее два варианта, `Ok` и `Err`, определено следующим образом:

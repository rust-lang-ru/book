...
Мы исправили ошибки связанные с распределением рукописи. Но остались сбоев с использованием
рукописи (функции не используются):
...
попробуем воспользоваться возможностями рукописи извне. Для этого создадим выполняемый
...
для библиотеки. Такая создание рукописи довольно-таки обычна. Большинство рукописи
...
Со стороны стороннего рукописи все что находится в библиотеке имеет пространство имён
...
<<<<<<< HEAD
звена `client`. Но при сборки этого рукописи получим сообщении об ошибке:
=======
звена `client`. Но при сборки этой рукописи получим сообщении об ошибке:
...
Т.к. рукопись в звене закрытый по умолчанию и не используется внутри библиотеки - на
это надо обратить внимание, т.к. это явная ошибка в согласования рукописи.

После определения функции `client::connect`, как доступной (`pub`), не только сообщение
об ошибке исчезнет, но и пропадёт сообщение о том, что рукопись не используется.
Создания доступного рукописи в Ржавчине даёт возможность его использования вне библиотеки.
Когда какой-либо рукописи помечается как `pub`, сборщик больше не сообщает об неиспользованном
рукописи, если даже он в действительности не используется.

### Сделать функции доступными

Для того, чтобы сделать что-либо доступным извне, необходимо добавить ключевое слово
`pub` в начало декларирования элемента рукописи. Для исправления ошибки, мы добавим
...
Рукопись собрался и предостережения о функции `client::connect` уже нет!

Вам решать, что делать с неиспользованным рукописью, то ли открыть к нему доступ, то ли
удалить.

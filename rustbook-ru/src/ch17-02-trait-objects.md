...
Эта ошибка даёт понять, что либо мы передаём в составляющая `Screen` что-то, что мы не собирались передавать и мы тогда должны передать другой вид, либо мы должны выполнить сущность `Draw` у вида `String`, чтобы `Screen` мог вызывать `draw` у него.

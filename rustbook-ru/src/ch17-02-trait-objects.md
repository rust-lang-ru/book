...
Эти правила написания должны выглядеть знакомым из наших дискуссий о том, как определять особенности в главе 10. Далее следуют новые правила написания: в приложении 17.4 определено устройства с именем `Screen`, которая содержит вектор с именем `components`. Этот вектор имеет вид `Box<dyn Draw>`, который и является особенность-предметом; это замена для любого вида внутри `Box` который выполняет особенность `Draw`.

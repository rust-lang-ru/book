...
Эти правила написания должны выглядеть знакомым из наших споров о том, как определять особенности в главе 10. Далее следуют новые правила написания: в приложении 17.4 определено устройства с именем `Screen`, которая содержит вектор с именем `components`. Этот вектор имеет вид `Box<dyn Draw>`, который и является сущность-предметом; это замена для любого вида внутри `Box` который выполняет сущность `Draw`.
...
<span class="caption">Приложение 17-9: Использование сущность-предметов для хранения значений разных видов, выполняющих один и тот же особенность</span>
...
<span class="caption">Приложение 17-10: Попытка использования вида, который не выполняет особенность для сущность-предмета</span>

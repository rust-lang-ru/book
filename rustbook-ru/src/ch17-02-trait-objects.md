...
В этом примере мы не будем реализовывать полноценную библиотеку GUI, но покажем, как некоторые её части будут сочетаться друг с другом. На момент написания библиотеки мы не имеем представления о всех тех типах, которые могут захотеть создать другие программисты и соответственно не можем их определить. Это означает, что `gui` должен поддерживать множество различных типов и мочь вызывать метод `draw` у каждого из них. Ему не нужно точно знать, что произойдет, когда мы вызовем метод `draw`, достаточно знать, что у того или иного объекта будет доступен этот метод для вызова.
...
Чтобы реализовать поведение, которое мы хотим видеть у `gui`, мы определим трейт `Draw`, который будет иметь один метод `draw`. Затем мы определим вектор, который принимает объект *trait*. Объект трейта указывает как на экземпляр типа, реализующего указанный нами трейт, так и на таблицу, используемую для поиска методов трейта на этом типе во время выполнения. Мы создаем трейт-объект, используя какой-либо указатель, например, ссылку `&` или умный указатель `Box<T>`, затем ключевое слово `dyn`, а затем указываем соответствующий трейт. (О том, почему для трейт-объектов необходимо использовать указатель, мы поговорим в главе 19 в разделе ["Динамически изменяемые типы и трейт `Sized`]). Мы можем использовать трейт-объекты в качестве замены универсального или конкретного типа. Где бы мы ни использовали трейт-объект, система типов Rust гарантирует во время компиляции, что любое значение, используемое в этом контексте, будет реализовывать трейт этого трейт-объекта. Следовательно, нам не нужно знать все возможные типы во время компиляции.
...
Этот синтаксис вам должен быть знаком по прошлому обсуждению определения трейтов в главе 10. Теперь перейдем к новому синтаксису: В листинге 17-4 определена структура с именем `Screen`, которая содержит вектор с названием `components`. Этот вектор представлен типом `Box<dyn Draw>`, который является трейт-объектом - он представляет собой обозначение любого типа внутри `Box`, реализующего трейт `Draw`.
...
Когда мы писали библиотеку, мы не предполагали, что кто-то добавит тип `SelectBox`, но наша реализация `Screen` смогла работать с новым типом и отрисовывать его, потому что `SelectBox` реализует признак `Draw`, что означает, что он реализует метод `draw`.

Эта концепция - опираться не на конкретный тип, а только на определённые факторы которым объект соответствует - похожа на принцип *duck typing* в динамически типизированных языках: если нечто ходит как утка и крякает как утка, то это, вероятно, и есть утка! В реализации `run` у `Screen` в листинге 17-5, `run` не нужно знать конкретный тип каждого компонента. Он не проверяет, является ли компонент экземпляром `Button` или `SelectBox`, он просто вызывает метод `draw` для компонента. Указав `Box<dyn Draw>` в качестве типа элементов вектора `components`, мы определили `Screen` как объекты, у которых можно вызвать метод `draw`.
...
Например, в листинге 17-10 показано, что произойдет, если мы попытаемся создать `Screen` с `String` в качестве его компонента:
...
Вспомните, в разделе "Производительность кода, использующего универсальные типы" в главе 10 наше обсуждение процесса мономорфизации, выполняемого компилятором, когда мы используем ограничения трейтов для универсальных типов: компилятор генерирует частные реализации функций и методов для каждого конкретного типа, который мы применяем для параметра универсального типа. Код, который получается в результате мономорфизации, выполняет статическую диспетчеризацию, то есть когда компилятор знает, какой метод вы вызываете во время компиляции. Это противоположно динамической диспетчеризации, когда компилятор не может определить во время компиляции, какой метод вы вызываете. В случае динамической диспетчеризации компилятор формирует код, который во время выполнения определит, какой метод нужно вызвать.
...
["Динамически изменяемые типы и трейт `Sized`]: ch19-04-advanced-types.html#dynamically-sized-types-and-the-sized-trait
...
Эти правила написания должны выглядеть знакомым из наших споров о том, как определять сущности в главе 10. Далее следуют новые правила написания: в приложении 17.4 определено устройства с именем `Screen`, которая содержит вектор с именем `components`. Этот вектор имеет вид данных `Box<dyn Draw>`, который и является сущность-предметом; это замена для любого вида данных внутри `Box` который использует сущность `Draw`.
...
<span class="caption">Приложение 17-6: Иная выполнение устройства <code>Screen</code> и способа <code>run</code>, используя обобщённый вид данных и ограничения сущности</span>

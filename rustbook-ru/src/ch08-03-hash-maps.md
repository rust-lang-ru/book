...
Мы не можем использовать переменные `имя_поля` и `значение_поля` после того, как их значения были переданы во владение в HashMap вызовом способа `insert`.

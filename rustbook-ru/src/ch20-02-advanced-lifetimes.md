...
Эта рукопись собирается. Следующая рукопись (19-14) объявляет способ (функцию), которая получает входное свойство `Context` и использует `Parser` для оценки писания.
...
<span class="caption">Listing 19-14: попытка добавить способ (функцию) `parse_context`,
...
заканчивается после завершения работы способы (функции). Но они должны продолжать жить далее.

Т.е. `Parser` и `context` должны пережить ( *outlive* ) завершение способы (функции) и
быть действительными после.

Давайте рассмотрим определение способы (функции) `parse` в 19-13 снова. Обратим внимание на
ярлык способа:
...
Неполадка этой способы (функции) в том, что время жизни образца данных `Parser` заканчивается после
завершения способы (функции) и время жизни свойства также.

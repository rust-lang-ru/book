...
`union` похожи на `struct`, но в каждый момент времени у каждого экземпляра union используется только одно поле. Объединения в основном используются для взаимодействия с объединениями в коде на C. Доступ к полям объединения небезопасен, потому что Rust не знает, какое поле заполнено в экземпляре объединения в данный момент времени. Подробнее об объединениях вы можете узнать в [справочнике] .
...
[справочнике]: ../reference/items/unions.html
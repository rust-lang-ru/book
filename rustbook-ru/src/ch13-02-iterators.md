...
<span class="caption">Listing 13-14: Using an iterator in a <code>for</code> loop</span>
...
<span class="caption">Listing 13-15: Calling the <code>next</code> method on an iterator</span>
...
<span class="caption">Listing 13-16: Calling the <code>sum</code> method to get the total of all items in the iterator</span>
...
<span class="caption">Listing 13-17: Calling the iterator adaptor <code>map</code> to create a new iterator</span>
...
<span class="caption">Listing 13-18: Calling the <code>map</code> method to create a new iterator and then calling the <code>collect</code> method to consume the new iterator and create a vector</span>
...
<span class="caption">Listing 13-19: Using the <code>filter</code> method with a closure that captures <code>shoe_size</code></span>
...
<span class="caption">Listing 13-20: Defining the <code>Counter</code> struct and a <code>new</code> function that creates instances of <code>Counter</code> with an initial value of 0 for <code>count</code></span>
...
<span class="caption">Listing 13-21: Implementing the <code>Iterator</code> trait on our <code>Counter</code> struct</span>
...
<span class="caption">Listing 13-22: Testing the functionality of the <code>next</code> method implementation</span>
...
<span class="caption">Listing 13-23: Using a variety of <code>Iterator</code> trait methods on our <code>Counter</code> iterator</span>

Note that `zip` produces only four pairs; the theoretical fifth pair `(5,
None)` is never produced because `zip` returns `None` when either of its input iterators return `None`.

...
Мы не можем использовать `v1_iter` нельзя использовать после вызова метода `sum`, потому что su`sum`m забирает по владение итератор у которого вызван метод.

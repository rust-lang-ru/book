...
В Rust итераторы *ленивые (lazy)*, то есть они не делают ничего, пока вы не вызовете специальные методы, потребляющие итератор, чтобы задействовать его. Например, код в листинге 13-10 создает итератор элементов вектора `v1`, вызывая метод `iter`, определенный у `Vec<T>`. Сам по себе этот код не делает ничего полезного.
...
В языках, стандартные библиотеки которых не предоставляют итераторы, вы, скорее всего, напишите эту же функциональность так - создав переменную cо значением 0, в цикле будете использовать её для получения элемента вектора по индексу, увеличивая её значение, пока оно не достигнет числа равного количеству элементов в векторе.

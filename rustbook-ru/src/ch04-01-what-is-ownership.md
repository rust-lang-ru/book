...
Чтобы поддерживать изменяемую, увеличивающуюся переменную вида `String`, необходимо выделять память в куче для всего содержимого, размер которого неизвестен во время сборки. Это означает, что:

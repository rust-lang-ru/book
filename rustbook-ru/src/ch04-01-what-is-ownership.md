...
Оператор двойное двоеточие `::` позволяет нам использовать пространство имён этой конкретной функции `from` под типом `String` , а не использовать какое-то имя вроде `string_from`. Мы обсудим этот синтаксис более подробно в разделе [«Синтаксис метода»].<!-- игнорировать --> Главы 5 и когда мы будем говорить о пространствах имён с модулями в [«Пути для обращения к элементу в дереве модулей»]<!-- игнорировать --> в Главе 7.

...
Оператор двойного двоеточия `::` позволяет нам использовать пространство имён функции `from` под типом `String`, вместо какого-то имени вроде `string_from`. Мы обсудим этот синтаксис более подробно в разделе [«Синтаксис метода»]<!-- ignore --> главы 5 и когда мы будем говорить о пространствах имён с модулями в [«Пути для обращения к элементу в дереве модулей»]<!-- ignore --> в главе 7.

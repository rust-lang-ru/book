...
Поле `sent_messages` теперь имеет вид данных `RefCell<Vec<String>>` вместо `Vec<String>`. В способы (функции) `new` мы создаём новый образец данных `RefCell<Vec<String>>` для пустого вектора.
...
После создания списков `a`, `b` и `c` мы хотим добавить 10 к значению в `value`. Для этого вызовем `borrow_mut` у `value`, которые используют способ (функцию) самостоятельного разыменования, о которой мы говорили в главе 5 (см. раздел ["Где находится приказчик `->`?"]<!-- ignore -->) во внутреннее значение `RefCell<T>`. Способ `borrow_mut` возвращает умный указатель `RefMut<T>`, и мы используя приказчик разыменования, изменяем внутреннее значение.

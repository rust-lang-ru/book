...
Эта проверочную рукопись определяет устройство `MockMessenger`, в которой есть поле `sent_messages` со значениями вида данных `Vec` из `String` для отслеживания сообщений, которые поручены устройству для отправки. Мы также определяем сопряженную функцию `new`, чтобы было удобно создавать новые образцы `MockMessenger`, которые создаются с пустым списком сообщений. Затем мы выполняем сущность `Messenger` для вида `MockMessenger`, чтобы передать `MockMessenger` в `LimitTracker`. В ярлыке способа `send` мы принимаем сообщение для передачи в качестве свойства и сохраняем его в `MockMessenger` внутри списка `sent_messages`.

...
Эту рукопись читается как "функция `bar` ничего не возвращает". Функции, которые ничего не возвращают, называются *рассеивающими функциями* (diverging functions). Мы не можем производить значения вида `!`, поэтому `bar` никогда ничего не вернёт.
...
В то время мы опустили некоторые подробности в этой рукописи. В главе 6 раздела ["Приказчик управления потоком `match`"](ch06-02-match.html#the-match-control-flow-operator)<!--  --> мы обсуждали, что все ветви `match` должны возвращать одинаковый вид. Например, следующий рукопись не работает:

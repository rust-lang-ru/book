...
По умолчанию Cargo создаёт библиотеки кода. Если при создании нового дела мы
...
Сейчас же мы не будем использовать данный возможности. поэтому просто удалим этот код.
...
Теперь создание нашего кода имеет вот такую устройство:
...
Сначала заменим код звена `client` на декларацию звена:
...
<span class="caption">Код 7-4: Ошибка при переносе кода вложенного звена `server`
...
* Если звено `foo` не имеет подчиненных звеньев, вы можете сохранить код звена в
файл *foo.rs*.
* Если звено `foo` имеет подзвено, вы должны перенести код звена в файл *foo/mod.rs*

...
Мы изучим использование связанных стопок `#[]` и `mod tests` в последующей разделы "Использование `super` для доступа к родительскому звену" этой главы.

Сейчас же мы не будем использовать данную возможности, поэтому просто удалим эту рукопись.
...
После определения звена, внутри узорчатых скобок пишем определения функции и
...
для какой-либо двоичной программы. Очень важная особенность звеньев - они могут быть вложенными. Это весьма удобно для разумной согласования рукописи.
Пример 7-2:
...
Теперь создание нашей рукописи имеет вот такую устройство:
...
Состоящая из звеньев устройство похоже на файловую систему. Мы можем использовать состоящую из звеньев
...
Если звенья имеют множество функций и эти функции длинные, было бы удобно разделить такую рукопись на несколько файлов.

Сначала заменим рукопись звена `client` на объявление звена:
...
Тут мы видим объявление звена. Этим мы сообщаем, что в другом месте есть определение звена `client`:
...
```
Обратите внимание, что вам не надо объявлять звено, т.к. вы уже объявили
его в файле *src/lib.rs*. Файл *src/client.rs* содержит составляющие звена `client`. Если же
вы и здесь напишите объявление звена `mod client`, то это будет значит, что внутри звена `client` есть звено `client`!
...
`bar` и `bar` не имеет подзвеньев, то у вас получится вот такое устройство
в папке *src*:

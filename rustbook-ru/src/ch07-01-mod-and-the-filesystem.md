...
Создадим основные разделы нашей библиотеки, которая будет предоставлять функциональные
...
```
Cargo  создаёт пустой проверка, чтобы показать как можно проверять функционал библиотеки.
Мы изучим использование синтаксических устройств `#[]` и `mod tests` в последующей
разделы "Использование `super` для доступа к родительскому модулю" этой главы.
...
Мы рассмотрим различные возможности согласования кода нашей библиотеки.
...
синтаксических устройств `network::connect` и `client::connect`.
...
для какой-либо двоичной программы. Очень важная особенностью модулей - они
могут быть вложенными. Это весьма удобно для логической согласования кода.
Пример 7-2:
...
Теперь создание нашего кода имеет вот такую структуру:
...
Логическая создание кода зависит от ваших задач.
...
его в файле *src/lib.rs*. Файл *src/client.rs* содержит составляющие модуля `client`. Если же
...
Сборка проекта пройдет успешно:
...
Эти сообщения сигнализируют нам, что наши функции нигде не используются. Пропренебрегаем
...
Далее, мы поговорим о модификаторе доступа `pub` и устраним сообщения о неполадках
в коде.

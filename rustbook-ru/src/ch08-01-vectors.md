...
Первый тип коллекции, который мы рассмотрим - это `Vec<T>`, также известный как *vector*. Векторы позволяют хранить более одного значения в одной структуре данных, которая помещает все значения рядом друг с другом в памяти. Векторы могут хранить значения только одного типа. Они полезны, когда у вас есть список товаров, например строки текста в файле или цены на товары в корзине.
...
Обратите внимание, что здесь мы добавили аннотацию типа. Поскольку мы не вставляем никаких значений в этот вектор, Rust не знает, какие элементы мы собираемся хранить. Это важный момент. Векторы реализованы с использованием обобщённых типов; мы рассмотрим, как использовать обобщенные типы с вашими собственными типами в главе 10. А пока знайте, что тип `Vec<T>` предоставляемый стандартной библиотекой, может содержать любой тип, и когда конкретный вектор содержит определенный тип, тип указаны в угловых скобках. В листинге 8-1 мы сообщили Rust, что `Vec<T>` в `v` будет содержать элементы типа `i32`.

В более реалистичном коде Rust часто может вывести тип значения, которое вы хотите сохранить, после того, как вы вставляете значения, поэтому вам редко нужно делать аннотацию этого типа. Чаще всего создается `Vec<T>` с начальными значениями, а Rust предоставляет `vec!` макрос для удобства. Макрос создаст новый вектор, содержащий заданные вами значения. В листинге 8-2 создается новый `Vec<i32>`, содержащий значения `1`, `2` и `3`. Целочисленный тип - `i32` потому что это целочисленный тип по умолчанию, как мы обсуждали в разделе [«Типы данных».]<!--  --> раздел главы 3.
...
Поскольку мы дали начальные значения `i32`, Rust может сделать вывод, что тип `v` - `Vec<i32>`, и аннотация типа не требуется. Далее мы посмотрим, как изменить вектор.
...
Когда вектор удаляется, все его содержимое также удаляется, что означает<br>те целые числа, которые он содержит, будут очищены. Это может показаться<br>простой момент, но может стать немного сложнее, когда вы начинаете<br>ввести ссылки на элементы вектора. Давайте займемся этим дальше!

### Чтение данных вектора

Теперь, когда вы знаете, как создавать, обновлять и уничтожать векторы, зная, как<br>читать их содержание хороший следующий шаг. Есть два способа сослаться на<br>значение хранится в векторе. В примерах мы аннотировали типы<br>значения, которые возвращаются из этих функций для большей ясности.
...
Код в Листинге 8-7 может выглядеть так, как будто он должен работать: зачем нужна ссылка<br>к первому элементу волнует, что меняется в конце вектора? это<br>ошибка связана с тем, как работают векторы: добавление нового элемента в конец<br>вектор может потребовать выделения новой памяти и копирования старых элементов в<br>новое пространство, если недостаточно места для размещения всех элементов рядом с каждым<br>другой, где вектор в настоящее время. В этом случае ссылка на первый<br>элемент будет указывать на освобожденную память. Правила заимствования мешают<br>программы в конечном итоге в этой ситуации.

> Примечание. Дополнительные сведения о реализации типа `Vec<T>` см. [В разделе «The Rustonomicon»].
...
[В разделе «The Rustonomicon»]: ../nomicon/vec.html
[«Следуя указателю на значение с помощью оператора разыменования»]: ../std/vec/struct.Vec.html
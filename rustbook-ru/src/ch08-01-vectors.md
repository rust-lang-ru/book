...
Первым типом коллекции, который мы разберём будет <code>Vec<T></code> известный как <em>вектор</em> (vector). Векторы позволяют сохранять более одного значения в одной структуре данных, сохраняющей элементы в памяти один за другим. Векторы могут сохранять данные только одного типа. Их удобно использовать, когда нужно сохранить список элементов, например, список текстовых строк в файле, или список цен товаров из корзине покупок.

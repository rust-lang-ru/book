...
В более реалистичном коде Rust часто может вывести тип значения, которое вы хотите сохранить, после того, как вы вставляете значения, поэтому вам редко нужно делать аннотацию этого типа. Чаще всего создаётся `Vec<T>` с начальными значениями, а Rust предоставляет `vec!` макрос для удобства. Макрос создаст новый вектор, содержащий заданные вами значения. В листинге 8-2 создаётся новый `Vec<i32>`, содержащий значения `1`, `2` и `3`. Целочисленный тип - `i32` потому что это целочисленный тип по умолчанию, как мы обсуждали в разделе [«Типы данных».]<!--  --> раздел главы 3.
...
Когда вектор удаляется, все его содержимое также удаляется, что означает<br>те целые числа, которые он содержит, будут очищены. Это может показаться<br>простой момент, но может стать немного сложнее, когда вы начинаете<br>ввести ссылки на элементы вектора. Давайте займёмся этим дальше!

...
Напомним, что `T` - это обобщенный вид данных, отображающий вид значения в `Some` исходе `Option`. Этот вид данных `T` также является возвращаемым видом функции `unwrap_or_else`: например, рукопись, вызывающая `unwrap_or_else` у `Option<String>`, получит `String`.
...
> Примечание: Функции также могут использовать все три сущности `Fn`. Если то, что мы хотим сделать, не требуя получения значения из среды, мы можем передать имя какой-либо функции вместо замыкания. Например, для значения `Option<Vec<T>>` мы можем вызвать `unwrap_or_else(Vec::new)`, чтобы получить новый пустой вектор, если значение окажется `None`. Сборщик самостоятельно использует любую из сущностей `Fn`, применимую для определения функции.

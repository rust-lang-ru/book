...
В этом разделе рассматривается пример использования `Option`, ещё одного перечисления, определённого в встроенной библиотеке. Вид данных `Option` кодирует очень распространённый задумка, в котором значение может быть чем-то, а может быть ничем.

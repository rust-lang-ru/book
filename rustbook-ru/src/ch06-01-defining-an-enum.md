...
Есть еще одно сходство между перечислениями и структурами: так же, как мы можем определять методы для структур с помощью `impl` блока, мы также можем определять методы для перечисления. Вот метод с именем `call`, который мы могли бы определить в нашем перечислении `Message` :
...
Проблема не в самой концепции, а в конкретной реализации. Таким образом, в Rust нет нулей (nulls), но есть перечисление, которое может закодировать концепцию наличия или отсутствия значения. Это перечисление `Option<T>` и оно [объяляется в стандартной библиотеке]<!-- ignore -->[ следующим образом:]
...
Короче говоря, поскольку `Option<T>` и `T` (где `T` может быть любым типом) относятся к разным типам, компилятор не позволит нам использовать значение `Option<T>` как если бы оно определенно допустимым значением. Например, этот код не компилируется, потому что он пытается добавить `i8` в `Option<i8>` :
...
Другими словами, вы должны преобразовать `Option<T>` в `T` прежде чем вы сможете выполнять операции с этим `T`. Как правило, это помогает выявить одну из наиболее распространенных проблем с null: предположение, что что-то не равно null, хотя на самом деле это так.
...
[объяляется в стандартной библиотеке]: ../std/option/enum.Option.html

...
Проблема не в самой концепции, а в конкретной реализации. Таким образом, в Rust нет null-значений, но есть перечисление, которое может закодировать концепцию наличия или отсутствия значения. Это перечисление `Option<T>` и оно [объявляется в стандартной библиотеке](../std/option/enum.Option.html)<!--  --> следующим образом:
...
Итак, как мы можем получить желанное значение типа `T`, упакованное в варианте `Some` типа `Option`, когда у нас на руках есть только значение типа `Option<T>`? `Option<T>` имеет большое количество методов, которые полезны в различных ситуациях; можно проверить их в [документации](../std/option/enum.Option.html)<!--  -->. Знакомство с методами в `Option<T>` будет чрезвычайно полезным в вашем путешествии по языку Rust.

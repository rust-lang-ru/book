...
Это определение компактно хранит данные. Оно подобно определению множества структур:
```rust
...
мы не можем легко определять функции, которые могли бы принимать любые типы сообщений,
...
В предыдущей секции, мы рассматривали как перечисление `IpAddr` позволяет нам
использовать систему типов Rust для архивирования информации.
Перечисление `Option` используется во многих местах, потому что оно реализует часто используемый

...
Проблема не в самой концепции, а в конкретной реализации. Таким образом, в Rust нет нулей null-значений, но есть перечисление, которое может закодировать концепцию наличия или отсутствия значения. Это перечисление `Option<T>` и оно [объявляется в стандартной библиотеке]<!--  --> следующим образом:

...
12-9. Если вы уверены в своих знаниях Ржавчина API, реализуйте способ `new` такого вида:
...
созданию нужного количества потоков и сохранить их в образце структуры `ThreadPool`
...
реализуем цикл `for` таким образом, чтобы о н создавал потоки и возвращал образец
`ThreadPool`:
...
создадим новую структуру, которая реализует подход *Рабочий* (*Worker*). *Рабочий*
...
Этот код собирается и сохраняет образцы структур. Пока мы никак не обрабатываем,

...
Владелец][join-владелец]<!-- ignore --> мы показали, как использовать способ `join` для вида `JoinHandle`, возвращаемого при вызове `std::thread::spawn`. Способ (функция) `trpl::join` похожа, но для фьючерсов. Когда вы передаете ей два фьючерса,

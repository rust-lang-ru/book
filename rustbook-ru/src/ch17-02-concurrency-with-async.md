...
которая является несогласованной версией API `thread::sleep`. Мы можем использовать их вместе,
...
несогласованные блоки компилируются в анонимные будущие, мы можем поместить каждый цикл в несогласованный
блок и заставить среду выполнения выполнить их оба до завершения с помощью функции `trpl::join`
.
...
<Listing number="17-8" caption="Используя `trpl::join` ожидание await двух анонимных futures" file-name="src/main.rs">
...
- Удалите несогласованный блок вокруг одного или обоих круговоротов.
- Ожидайте каждый несогласованный блок сразу после его определения.
- Оберните только первый цикл в несогласованный блок и ожидайте полученное будущее
после тела второго круговорота.

Для дополнительной сложности попробуйте выяснить, какой будет вывод в каждом случае _до_ запуска кода!
...
Листинге 17-9 мы начнем всего с одного несогласованного блока — _не_ порождая
...
Здесь мы используем `trpl::channel`, несогласованную версию API с несколькими производителями и одним потребителем, которую мы использовали с потоками в Главе 16. Несогласованное
...
> кода, и, таким образом, где переходить между согласованным и несогласованным кодом. В большинстве несогласованных
...
блоке порядок, в котором ключевые слова `await` появляются в коде, также является порядком, в котором они выполняются при запуске программы.
...
<Listing number="17-11" caption="Разделение `send` и `recv` на отдельные `async` блоки и ожидание будущих событий для этих блоков" file-name="src/main.rs">
...
несогласованный блок, он был бы удален, как только этот блок закончится. В разделе Главы 13
...
`async move`. Когда мы запускаем _эту_ версию кода, она корректно завершает работу
после отправки и получения последнего сообщения.

<Listing number="17-12" caption="Пересмотр кода из приложения 17-11, который корректно завершает работу после завершения" file-name="src/main.rs">
...
`tx1` в этот блок так же, как мы делали это раньше с `tx`. Затем, позже, мы перемещаем
исходный `tx` в _новый_ несогласованный блок, где мы отправляем больше сообщений с
немного меньшей задержкой. Мы случайно помещаем этот новый несогласованный блок после несогласованного
блока для получения сообщений, но он мог бы идти и перед ним. Ключевым является
порядок, в котором ожидаются фьючерсы, а не порядок, в котором они создаются.

Оба несогласованных блока для отправки сообщений должны быть блоками `async move`,

...
Правила написания `impl Trait` работает для простых случаев, но на самом деле является связанным сахаром для более длинной вида данных, которая называется *ограничением сущности* (trait bound); это выглядит так:
...
Обобщённый вид данных `T` указан для видов свойств `item1` и `item2` и ограничивает функцию так, что определенные значения видов данных переданные переменнойми для `item1` и `item2` должны быть одинаковыми.
...
Правила написания `+` также допустим с ограничениями сущности для обобщённых видов данных:
...
Возврат либо `NewsArticle` либо `Tweet` не допускается из-за ограничений того, как выполнен правила написания `impl Trait` в сборщике. Мы рассмотрим, как написать функцию с таким поведением в разделе ["Использование предметов сущностей, которые разрешены для значений или разных видов данных"]<!--  --> Главы 17.

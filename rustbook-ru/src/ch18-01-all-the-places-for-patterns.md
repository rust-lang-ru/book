...
Код в листинге 18-1 показывает последовательность проверок нескольких условий, определяющих каким должен быть цвет фона. В данном примере, мы создали переменные с предопределенными значениями, которые в реальной программе могли получить из пользовательского ввода.
